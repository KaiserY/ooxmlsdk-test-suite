...
### 2026-08-16 Static namespace read path
...
The command was run three times without rebuilding between measured runs or
changing Criterion's sample size and measurement time. The table below is the
arithmetic mean of the three `median.point_estimate` values written to
Criterion's `estimates.json`.
The XML write and round-trip benchmarks continue to use `to_xml()` and remain
comparable with the previous documented runs.

Current state under test:

- Thread one `ReadContext` through generated `read_inner` calls while keeping
  element child dispatch on the existing local-name fast path.
- Keep generated attribute QName matching as the first path and resolve static
  namespace aliases only on an unmatched prefixed attribute.
- Generate `XmlKnownNamespace` entries and Strict, Transitional, and supported
  preview aliases statically; retain only non-canonical or unknown declarations
  in the runtime context.
- Represent MCE namespace lists with typed namespaces and replace broad
  local-name attribute matching with the two source-backed `ReadAliases`.
- Use the same generic XML implementation for WordprocessingML,
  SpreadsheetML, and PresentationML without document-family branches.

Three-run average medians:

| Benchmark | Read slice | Read cursor | Read bufreader | Write | Round-trip |
| --- | ---: | ---: | ---: | ---: | ---: |
| `xml/word/document_hello_world` | 3.410 us | 3.969 us | 4.062 us | 564.72 ns | 4.163 us |
| `xml/word/document_complex0` | 1.420 ms | 1.689 ms | 1.715 ms | 121.87 us | 1.577 ms |
| `xml/sheet/worksheet_no_ext_data_b1_sheet1` | 5.592 ms | 7.141 ms | 7.334 ms | 454.70 us | 6.163 ms |
| `xml/slides/presentation` | 8.548 us | 10.708 us | 10.792 us | 1.152 us | 9.951 us |

Compared with the previous documented three-run average after the XML string
read/write cleanup:

| Benchmark | Read slice | Read cursor | Read bufreader | Write | Round-trip |
| --- | ---: | ---: | ---: | ---: | ---: |
| `xml/word/document_hello_world` | flat (-0.5%) | improved (-1.5%) | improved (-1.9%) | improved (-8.0%) | improved (-3.6%) |
| `xml/word/document_complex0` | improved (-2.2%) | flat (-0.7%) | improved (-2.3%) | improved (-10.6%) | improved (-3.5%) |
| `xml/sheet/worksheet_no_ext_data_b1_sheet1` | improved (-3.8%) | flat (+0.1%) | noise (+0.4%) | improved (-6.2%) | improved (-2.9%) |
| `xml/slides/presentation` | improved (-1.8%) | improved (-3.6%) | improved (-4.7%) | improved (-17.6%) | improved (-5.3%) |

Conclusion: keep the static namespace read path. All slice reads and all
round-trips remain faster than the previous documented run. The worksheet
cursor and bufreader differences are below 0.4% and are treated as run noise;
the other stream reads are flat or improved. The write results also remain
lower, although the three hello-world write medians had a wider 11% range and
the worksheet write medians a 5% range. This is a current-state snapshot across
all changes since the July run, so the comparison records the repository trend
rather than attributing every improvement to namespace handling alone.
