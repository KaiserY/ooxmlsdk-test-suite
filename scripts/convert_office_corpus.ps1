...
$exportProfile = "office-fixed-format-print-no-macros-no-links-v3"
$exportConfiguration = [ordered]@{
    word = [ordered]@{
        optimize_for = "print"
        range = "all-document"
        item = "document-content-no-markup"
        include_document_properties = $false
        keep_irm_and_labels = $false
        ...
        update_links_at_print = $false
        print_backgrounds = $false
        print_drawing_objects = $true
        print_field_codes = $false
        print_hidden_text = $false
    }
    excel = [ordered]@{
        quality = "standard"
        include_document_properties = $false
        honor_print_areas = $true
        range = "all-pages"
    }
    powerpoint = [ordered]@{
        intent = "print"
...
$wordOptionsBeforeAutomation = $null
...
function Get-ReferenceEnvironment {
    ...
    return [ordered]@{
        ...
        dependencies = @(
            ...
            "default-paper",
            "explicit-office-export-configuration"
        )
    }
}
...
function Set-WordAutomationOptions {
    param($Application)

    $options = $Application.Options
    $script:wordOptionsBeforeAutomation = [ordered]@{
        UpdateFieldsAtPrint = [bool]$options.UpdateFieldsAtPrint
        UpdateLinksAtOpen = [bool]$options.UpdateLinksAtOpen
        UpdateLinksAtPrint = [bool]$options.UpdateLinksAtPrint
        PrintBackgrounds = [bool]$options.PrintBackgrounds
        PrintDrawingObjects = [bool]$options.PrintDrawingObjects
        PrintFieldCodes = [bool]$options.PrintFieldCodes
        PrintHiddenText = [bool]$options.PrintHiddenText
    }
    try {
        $options.UpdateFieldsAtPrint = $false
        $options.UpdateLinksAtOpen = $false
        $options.UpdateLinksAtPrint = $false
        $options.PrintBackgrounds = $false
        $options.PrintDrawingObjects = $true
        $options.PrintFieldCodes = $false
        $options.PrintHiddenText = $false
    }
    catch {
        $options.UpdateFieldsAtPrint = $script:wordOptionsBeforeAutomation.UpdateFieldsAtPrint
        $options.UpdateLinksAtOpen = $script:wordOptionsBeforeAutomation.UpdateLinksAtOpen
        $options.UpdateLinksAtPrint = $script:wordOptionsBeforeAutomation.UpdateLinksAtPrint
        $options.PrintBackgrounds = $script:wordOptionsBeforeAutomation.PrintBackgrounds
        $options.PrintDrawingObjects = $script:wordOptionsBeforeAutomation.PrintDrawingObjects
        $options.PrintFieldCodes = $script:wordOptionsBeforeAutomation.PrintFieldCodes
        $options.PrintHiddenText = $script:wordOptionsBeforeAutomation.PrintHiddenText
        $script:wordOptionsBeforeAutomation = $null
        throw
    }
    finally {
        Release-ComObject $options
    }
}

function Restore-WordAutomationOptions {
    param($Application)

    if ($null -eq $script:wordOptionsBeforeAutomation) {
        return
    }
    $options = $Application.Options
    try {
    ...
    switch ($Family) {
        "Word" {
            ...
            try {
                ...
                Set-WordAutomationOptions $application
            }
            catch {
...
function Stop-OfficeApplication {
    ...
    if ($Family -eq "Word") {
        try {
            Restore-WordAutomationOptions $Application
        }
        catch {
            Write-Warning ("Failed to restore Word options: {0}" -f $_.Exception.Message)
        }
    }
    try {
        $Application.Quit()
...
function Export-WithWord {
    ...
    try {
        ...
        # wdExportFormatPDF, print quality, whole document, document content,
        # no metadata/labels/bookmarks, tagged structure, licensed-font bitmap
        # fallback, and ordinary (non-PDF/A) output.
...
function Export-WithExcel {
    ...
    try {
        ...
        # xlTypePDF, standard quality, no metadata, authored print areas, all
        # pages, and no viewer launch.
...
function Export-WithPowerPoint {
    ...
    try {
        ...
        # PowerShell cannot marshal Nothing for ExportAsFixedFormat's required
        # PrintRange COM parameter. An explicit 1..Count range is equivalent to
        # ppPrintAll here; PrintHiddenSlides still controls hidden slides.
        $printRange = $presentation.PrintOptions.Ranges.Add(1, $presentation.Slides.Count)
        # ppFixedFormatTypePDF, print intent, unframed slides, vertical handout
        # order (irrelevant for slide output), visible slides only, no metadata
        # or labels, tagged structure, licensed-font bitmap fallback, non-PDF/A.
