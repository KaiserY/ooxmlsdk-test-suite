...
pub fn assert_package_file_round_trip(path: &Path, file_name: &str) {
    ...
    match kind {
        ...
        panic!("round-trip failed for {file_name} while opening original wordprocessing package {path:?}: {err:?}");
            ...
            let first_saved_bytes =
                save_package_generation(&original, file_name, "wordprocessing", "first");
            let mut first_reopened = WordprocessingDocument::new_with_settings(
                Cursor::new(first_saved_bytes.as_slice()),
                eager_open_settings(),
            ).unwrap_or_else(|err| {
        panic!("round-trip failed for {file_name} while reopening first saved wordprocessing package: {err:?}");
            });
            assert_wordprocessing_document_round_trip(&original, &first_reopened);
            assert_doc_sample_zip_equivalent(&original_bytes, &first_saved_bytes, file_name);
            drop(original_bytes);
            clear_deep_recursive_word_tables_for_known_fixture(&mut original, file_name);
            drop(original);

            let second_saved_bytes =
                save_package_generation(&first_reopened, file_name, "wordprocessing", "second");
            let mut second_reopened = WordprocessingDocument::new_with_settings(
                Cursor::new(second_saved_bytes.as_slice()),
                eager_open_settings(),
            )
            .unwrap_or_else(|err| {
                panic!("round-trip failed for {file_name} while reopening second saved wordprocessing package: {err:?}");
            });
            assert_wordprocessing_document_round_trip(&first_reopened, &second_reopened);
            clear_deep_recursive_word_tables_for_known_fixture(&mut first_reopened, file_name);
            clear_deep_recursive_word_tables_for_known_fixture(&mut second_reopened, file_name);
            drop(first_reopened);
            drop(second_reopened);
            assert_sdk_saved_package_stable(&first_saved_bytes, &second_saved_bytes, file_name);
        }
        ...
        panic!("round-trip failed for {file_name} while opening original spreadsheet package {path:?}: {err:?}");
            ...
            let first_saved_bytes =
                save_package_generation(&original, file_name, "spreadsheet", "first");
            let first_reopened = SpreadsheetDocument::new_with_settings(
                Cursor::new(first_saved_bytes.as_slice()),
                eager_open_settings(),
            )
        .unwrap_or_else(|err| {
          panic!(
            "round-trip failed for {file_name} while reopening first saved spreadsheet package: {err:?}"
          );
            });
            assert_spreadsheet_document_round_trip(&original, &first_reopened);
            assert_doc_sample_zip_equivalent(&original_bytes, &first_saved_bytes, file_name);
            drop(original_bytes);
            drop(original);

            let second_saved_bytes =
                save_package_generation(&first_reopened, file_name, "spreadsheet", "second");
            let second_reopened = SpreadsheetDocument::new_with_settings(
                Cursor::new(second_saved_bytes.as_slice()),
                eager_open_settings(),
            )
            .unwrap_or_else(|err| {
                panic!(
                    "round-trip failed for {file_name} while reopening second saved spreadsheet package: {err:?}"
                );
            });
            assert_spreadsheet_document_round_trip(&first_reopened, &second_reopened);
            drop(first_reopened);
            drop(second_reopened);
            assert_sdk_saved_package_stable(&first_saved_bytes, &second_saved_bytes, file_name);
        }
        ...
        panic!("round-trip failed for {file_name} while opening original presentation package {path:?}: {err:?}");
            ...
            let first_saved_bytes =
                save_package_generation(&original, file_name, "presentation", "first");
            let first_reopened = PresentationDocument::new_with_settings(
                Cursor::new(first_saved_bytes.as_slice()),
                eager_open_settings(),
            )
        .unwrap_or_else(|err| {
          panic!(
            "round-trip failed for {file_name} while reopening first saved presentation package: {err:?}"
          );
            });
            assert_presentation_document_round_trip(&original, &first_reopened);
            assert_doc_sample_zip_equivalent(&original_bytes, &first_saved_bytes, file_name);
            drop(original_bytes);
            drop(original);

            let second_saved_bytes =
                save_package_generation(&first_reopened, file_name, "presentation", "second");
            let second_reopened = PresentationDocument::new_with_settings(
                Cursor::new(second_saved_bytes.as_slice()),
                eager_open_settings(),
            )
            .unwrap_or_else(|err| {
                panic!(
                    "round-trip failed for {file_name} while reopening second saved presentation package: {err:?}"
                );
            });
            assert_presentation_document_round_trip(&first_reopened, &second_reopened);
            drop(first_reopened);
            drop(second_reopened);
            assert_sdk_saved_package_stable(&first_saved_bytes, &second_saved_bytes, file_name);
        }
...
fn save_package_generation<P: SdkPackage>(
    package: &P,
    file_name: &str,
    document_kind: &str,
    generation: &str,
    ...
    ooxmlsdk::parts::save_package(package, &mut buffer).unwrap_or_else(|err| {
        panic!(
            "round-trip failed for {file_name} while saving {generation} {document_kind} package: {err:?}"
        );
    });
    buffer.into_inner()
}
    ...
    let result = match kind {
        ...
        DocSampleKind::Spreadsheet => {
            SpreadsheetDocument::new_from_file_with_settings(path, eager_open_settings())
                .and_then(|package| package.workbook_part()?.root_element(&package).map(|_| ()))
        }
        DocSampleKind::Presentation => {
            PresentationDocument::new_from_file_with_settings(path, eager_open_settings()).and_then(
                |package| {
                    package
                        .presentation_part()?
                        .root_element(&package)
    ...
    P: SdkPackage,
{
    ...
    assert_eq!(
        all_part_signature(original, original.get_all_parts().collect()),
        all_part_signature(roundtripped, roundtripped.get_all_parts().collect()),
    ...
    T: SdkPart,
{
    ...
    assert_eq!(
        all_part_signature(
            original_package,
            original_part.get_all_parts(original_package).collect(),
        ),
        all_part_signature(
            roundtripped_package,
            roundtripped_part
    ...
    parts
        ...
        .map(|pair| {
            (
                ...
                pair.part.path(package).unwrap().to_string(),
            )
        })
        .collect()
}

fn all_part_signature<P: SdkPackage>(package: &P, parts: Vec<PartRef>) -> Vec<String> {
    parts
        .into_iter()
        .map(|part| part.path(package).unwrap().to_string())
        .collect()
}
...
fn assert_doc_sample_zip_equivalent(original: &[u8], roundtripped: &[u8], file_name: &str) {
    ...
    assert_relationship_graph_equal(&original, &roundtripped, file_name);
...
fn assert_sdk_saved_package_stable(first_saved: &[u8], second_saved: &[u8], file_name: &str) {
    let first_saved = read_zip_entries(first_saved, file_name);
    let second_saved = read_zip_entries(second_saved, file_name);
    assert_relationship_graph_equal(&first_saved, &second_saved, file_name);

    let mut errors = Vec::new();
    for name in first_saved.keys() {
        if !second_saved.contains_key(name) {
            errors.push(format!("second save is missing zip entry: {name}"));
        }
    }
    for name in second_saved.keys() {
        if !first_saved.contains_key(name) {
            errors.push(format!("second save added zip entry: {name}"));
        }
    }

    for (name, first_bytes) in &first_saved {
        let Some(second_bytes) = second_saved.get(name) else {
            continue;
        };
        if should_compare_entry_as_xml(name, first_bytes, second_bytes) || is_psmdcp_entry(name) {
            errors.extend(strict_xml_equivalence_errors(
                first_bytes,
                second_bytes,
                file_name,
                name,
            ));
        } else if first_bytes != second_bytes {
            errors.push(format!(
                "{name}: binary entry changed between SDK saves: first {} bytes, second {} bytes",
                first_bytes.len(),
                second_bytes.len()
            ));
        }
    ...
    assert!(
        errors.is_empty(),
        "SDK package output is not stable after a second eager reopen/save for {file_name}\n{}",
        format_doc_sample_errors(&errors)
    );
}
...
struct RelationshipEdgeSignature {
    source: String,
...
fn assert_relationship_graph_equal(
    original: &BTreeMap<String, Vec<u8>>,
    roundtripped: &BTreeMap<String, Vec<u8>>,
    file_name: &str,
) {
    assert_eq!(
        relationship_graph_signature(original, file_name, "original"),
        relationship_graph_signature(roundtripped, file_name, "roundtripped"),
        "complete relationship graph differs for {file_name}"
    );
}

fn relationship_graph_signature(
    entries: &BTreeMap<String, Vec<u8>>,
    file_name: &str,
    side: &str,
) -> Vec<RelationshipEdgeSignature> {
    ...
    for (entry_name, bytes) in entries {
        let Some(source) = relationship_source_path(entry_name) else {
            continue;
        };
        let nodes = canonicalize_xml(bytes, CanonicalOptions::strict(), file_name, entry_name);
        let roots = nodes
            .iter()
            .filter_map(|node| match node {
                ...
                XmlNode::Text(text) => panic!(
                    "non-whitespace root text in {side} relationship entry for {file_name}:{entry_name}: {text:?}"
                ),
            })
            .collect::<Vec<_>>();
        let [root] = roots.as_slice() else {
            panic!(
                "expected one root in {side} relationship entry for {file_name}:{entry_name}, found {}",
                roots.len()
            );
        };
        assert_eq!(
            root.name, RELATIONSHIPS_NAME,
            "unexpected root in {side} relationship entry for {file_name}:{entry_name}"
        );

        for child in &root.children {
            let relationship = match child {
                XmlNode::Element(element) if element.name == RELATIONSHIP_NAME => element,
                XmlNode::Text(text) if text.trim().is_empty() => continue,
                XmlNode::Declaration(_) => panic!(
                    "nested declaration in {side} relationship entry for {file_name}:{entry_name}"
                ),
                XmlNode::Element(element) => panic!(
                    "unexpected child {} in {side} relationship entry for {file_name}:{entry_name}",
                    element.name
                ),
                XmlNode::Text(text) => panic!(
                    "non-whitespace child text in {side} relationship entry for {file_name}:{entry_name}: {text:?}"
                ),
            };
            let relationship_attr = |attr_name| {
                relationship
                    .attrs
                    .iter()
                    .find_map(|(name, value)| (name == attr_name).then(|| value.clone()))
            };
            let relationship_id = relationship_attr("Id");
            let relationship_type = relationship_attr("Type");
            let target = relationship_attr("Target");
            let target_mode = relationship
                .attrs
                .iter()
                .find_map(|(name, value)| (name == "TargetMode").then_some(value.as_str()))
                .unwrap_or("Internal");
            let resolved_target = target.as_deref().and_then(|target| {
                (target_mode == "Internal" && !target.eq_ignore_ascii_case("NULL")).then(|| {
                    let parent = relationship_source_parent_path(&source);
                    ooxmlsdk::common::resolve_relationship_target_path(&parent, target)
                })
            });

            signature.push(RelationshipEdgeSignature {
                source: source.clone(),
                relationship_id,
                relationship_type,
...
fn relationship_source_path(entry_name: &str) -> Option<String> {
    if entry_name == "_rels/.rels" {
        return Some("/".to_string());
    }
    let without_suffix = entry_name.strip_suffix(".rels")?;
    if let Some(source) = without_suffix.strip_prefix("_rels/") {
        return Some(source.to_string());
    }
    let (parent, source_name) = without_suffix.rsplit_once("/_rels/")?;
    Some(format!("{parent}/{source_name}"))
}

fn relationship_source_parent_path(source: &str) -> String {
    if source == "/" {
        return String::new();
    }
    source
...
fn strict_xml_equivalence_errors(
    first_saved: &[u8],
    second_saved: &[u8],
    file_name: &str,
    entry_name: &str,
) -> Vec<String> {
    let options = CanonicalOptions::strict();
    let first_saved = canonicalize_xml(first_saved, options, file_name, entry_name);
    let second_saved = canonicalize_xml(second_saved, options, file_name, entry_name);
    let strict_errors = compare_xml_documents(&first_saved, &second_saved);
    if strict_errors.is_empty() {
        return Vec::new();
    }

    let mut errors = vec![format!(
        "{entry_name}: XML changed between first and second SDK saves"
    )];
    if let Some(summary) = summarize_xml_mismatch_cause(&strict_errors) {
        errors.push(format!("{entry_name}: first structural cause: {summary}"));
    }
    for error in strict_errors.iter().take(24) {
        errors.push(format!("{entry_name}: strict: {error}"));
    }
    if strict_errors.len() > 24 {
        errors.push(format!(
            "{entry_name}: strict: ... {} additional XML mismatches omitted",
            strict_errors.len() - 24
        ));
    }
    errors
}
...
struct SchemaFloatRules {
    ...
    #[serde(default)]
    compat_attrs: Vec<SchemaFloatAttrRule>,
...
struct CanonicalOptions {
    ...
    ignore_pandoc_ns0_reference_synthetic_sectpr_type_attr: bool,
    ...
    normalize_word_settings_duplicate_proof_state: bool,
    ...
    ignore_word_font_table_element_whitespace_text_nodes: bool,
    sort_word_font_table_font_children: bool,
    sort_word_style_children: bool,
    sort_word_style_table_cell_properties_children: bool,
    sort_word_style_table_cell_margin_children: bool,
    sort_math_delimiter_properties_children: bool,
...
impl CanonicalOptions {
    fn strict() -> Self {
        Self {
            ...
            ignore_pandoc_ns0_reference_synthetic_sectpr_type_attr: false,
            ...
            normalize_word_settings_duplicate_proof_state: false,
            ...
            ignore_word_font_table_element_whitespace_text_nodes: false,
            sort_word_font_table_font_children: false,
            sort_word_style_children: false,
            sort_word_style_table_cell_properties_children: false,
            sort_word_style_table_cell_margin_children: false,
            sort_math_delimiter_properties_children: false,
    ...
    fn relaxed_for_entry(file_name: &str, entry_name: &str) -> Self {
        Self {
            ...
            ignore_pandoc_ns0_reference_synthetic_sectpr_type_attr: file_name
                .ends_with("Pandoc/test/docx/ns0-reference.docx")
            ...
            normalize_word_settings_duplicate_proof_state: is_word_settings_entry(entry_name),
            ...
            ignore_word_font_table_element_whitespace_text_nodes: is_word_font_table_entry(
                entry_name,
            ),
            ...
            sort_word_style_table_cell_properties_children: is_word_styles_entry(entry_name),
            sort_word_style_table_cell_margin_children: is_word_styles_entry(entry_name),
            sort_math_delimiter_properties_children: is_word_xml_entry(entry_name),
    ...
    fn describe(self) -> String {
        ...
        if self.ignore_pandoc_ns0_reference_synthetic_sectpr_type_attr {
            enabled.push("Pandoc ns0 reference synthetic sectPr type attr");
        }
        ...
        if self.normalize_word_settings_duplicate_proof_state {
            enabled.push("word settings duplicate proofState");
        }
        ...
        if self.ignore_word_font_table_element_whitespace_text_nodes {
            enabled.push("word font table element whitespace text nodes");
        }
        ...
        if self.sort_word_style_table_cell_properties_children {
            enabled.push("word style table cell properties child order");
        }
        if self.sort_word_style_table_cell_margin_children {
            enabled.push("word style table cell margin child order");
        }
        if self.sort_math_delimiter_properties_children {
            enabled.push("math delimiter properties child order");
        }
    ...
    while let Some(task) = tasks.pop() {
        match task {
            ...
            NormalizeTask::Finish(mut element) => {
                ...
                if options.ignore_word_font_table_element_whitespace_text_nodes
                    && is_word_font_table_element_only_root(&element.name)
                {
                    element.children.retain(
                        |child| !matches!(child, XmlNode::Text(text) if is_xml_whitespace(text)),
                    );
                }
                ...
                {
                    if options.normalize_word_settings_duplicate_proof_state {
                        normalize_word_settings_duplicate_proof_state(&mut element.children);
                    }
                    element.children.sort_by_key(xml_node_structural_sort_key);
                }
                ...
                if options.sort_word_style_table_cell_properties_children {
                    normalize_word_style_table_cell_properties_children(
                        &element.name,
                ...
                if options.sort_math_delimiter_properties_children
                    && is_math_delimiter_properties_order_relaxed_root(&element.name)
                {
                    normalize_math_delimiter_properties_child_order(&mut element.children);
                }
...
fn normalize_word_style_table_cell_properties_children(
    parent_name: &str,
    children: &mut [XmlNode],
) {
    let rank = match parent_name {
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}style" => {
            word_style_table_cell_properties_child_rank
        }
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tblStylePr" => {
            word_table_style_override_cell_properties_child_rank
        }
        _ => return,
    };
    ...
    for child in children {
        ...
        if child.name != "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tcPr"
            || child
                .children
                .iter()
                .any(|item| rank(xml_node_name(item)).is_none())
        {
            continue;
        }
        child
            .children
            .sort_by_key(|item| rank(xml_node_name(item)).unwrap_or(u16::MAX));
    }
}

fn word_style_table_cell_properties_child_rank(name: Option<&str>) -> Option<u16> {
    Some(match name? {
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}shd" => 0,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}noWrap" => 1,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tcMar" => 2,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}vAlign" => 3,
        _ => return None,
    })
}

fn word_table_style_override_cell_properties_child_rank(name: Option<&str>) -> Option<u16> {
    Some(match name? {
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tcBorders" => 0,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}shd" => 1,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}noWrap" => 2,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tcMar" => 3,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}vAlign" => 4,
        _ => return None,
    })
}
...
fn normalize_math_delimiter_properties_child_order(children: &mut [XmlNode]) {
    if children
        .iter()
        .any(|child| math_delimiter_properties_child_rank(xml_node_name(child)).is_none())
    {
        return;
    }

    children.sort_by_key(|child| {
        math_delimiter_properties_child_rank(xml_node_name(child)).unwrap_or(u16::MAX)
    });
}

fn math_delimiter_properties_child_rank(name: Option<&str>) -> Option<u16> {
    Some(match name? {
        "{http://schemas.openxmlformats.org/officeDocument/2006/math}begChr" => 0,
        "{http://schemas.openxmlformats.org/officeDocument/2006/math}sepChr" => 1,
        "{http://schemas.openxmlformats.org/officeDocument/2006/math}endChr" => 2,
        "{http://schemas.openxmlformats.org/officeDocument/2006/math}grow" => 3,
        "{http://schemas.openxmlformats.org/officeDocument/2006/math}shp" => 4,
        "{http://schemas.openxmlformats.org/officeDocument/2006/math}ctrlPr" => 5,
        _ => return None,
    })
}
...
fn normalize_word_settings_duplicate_proof_state(children: &mut Vec<XmlNode>) {
    const PROOF_STATE_NAME: &str =
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}proofState";

    let proof_states = children
        .iter()
        .filter(|child| xml_node_name(child) == Some(PROOF_STATE_NAME))
        .collect::<Vec<_>>();
    let Some(first) = proof_states.first() else {
        return;
    };
    if proof_states.len() < 2 || proof_states.iter().skip(1).any(|child| *child != *first) {
        return;
    }

    let mut found = false;
    children.retain(|child| {
        if xml_node_name(child) != Some(PROOF_STATE_NAME) {
            return true;
        }
        if found {
            false
...
fn normalize_chart_schema_child_order(parent_name: &str, children: &mut [XmlNode]) {
    ...
    if parent_name == CHART_DATA_LABELS_NAME
        && children
            .iter()
            .any(|child| rank_fn(xml_node_name(child)).is_none())
    {
        return;
    }
...
fn chart_child_rank_fn(parent_name: &str) -> Option<ChartChildRankFn> {
    match parent_name {
        ...
        CHART_DATA_LABELS_NAME => Some(chart_data_labels_child_rank),
...
fn chart_data_labels_child_rank(name: Option<&str>) -> Option<u16> {
    match name? {
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}dLbl" => Some(0),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}delete" => Some(1),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}numFmt" => Some(2),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}spPr" => Some(3),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}txPr" => Some(4),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}dLblPos" => Some(5),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showLegendKey" => Some(6),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showVal" => Some(7),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showCatName" => Some(8),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showSerName" => Some(9),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showPercent" => Some(10),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showBubbleSize" => Some(11),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}separator" => Some(12),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}showLeaderLines" => Some(13),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}leaderLines" => Some(14),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}layout" => Some(15),
        "{http://schemas.openxmlformats.org/drawingml/2006/chart}extLst" => Some(16),
        _ => None,
    }
}
...
fn parse_xml_node(
    ...
    for (key, value) in &raw_attrs {
        ...
        if options.ignore_pandoc_ns0_reference_synthetic_sectpr_type_attr
            && value == "continuous"
            && name == "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}sectPr"
            && expanded_key == "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}type"
        {
            continue;
        }
...
fn is_word_xml_entry(entry_name: &str) -> bool {
    entry_name.starts_with("word/") && entry_name.ends_with(".xml")
}
...
fn is_word_font_table_element_only_root(name: &str) -> bool {
    matches!(
        name,
        "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}fonts"
            | "{http://schemas.openxmlformats.org/wordprocessingml/2006/main}font"
    )
}

fn is_xml_whitespace(text: &str) -> bool {
    text.chars()
        .all(|character| matches!(character, '\u{20}' | '\u{9}' | '\u{a}' | '\u{d}'))
}
...
fn is_math_delimiter_properties_order_relaxed_root(name: &str) -> bool {
    name == "{http://schemas.openxmlformats.org/officeDocument/2006/math}dPr"
}
...
const CHART_DATA_LABELS_NAME: &str =
...
fn schema_float_kind_for_attr(element_name: &str, attr_name: &str) -> Option<SchemaFloatKind> {
    schema_float_rules()
        .attrs
        .iter()
        .chain(&schema_float_rules().compat_attrs)
...
mod tests {
    ...
    use super::{
        relationship_graph_signature, relationship_source_path, strict_xml_equivalence_errors,
        xml_equivalence_errors,
    };

    const FILE_NAME: &str = "Apache-POI/test-data/spreadsheet/sample.xlsx";
    const ENTRY_NAME: &str = "xl/charts/chart1.xml";
    const WORD_FILE_NAME: &str = "Pandoc/test/docx/sample.docx";
    const PANDOC_NS0_FILE_NAME: &str = "Pandoc/test/docx/ns0-reference.docx";

    #[test]
    fn relationship_graph_preserves_multiple_edges_to_the_same_target() {
        let mut entries = BTreeMap::new();
        entries.insert(
            "_rels/.rels".to_string(),
            br#"<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/><Relationship Id="rId2" Type="http://example.com/alias" Target="word/document.xml"/></Relationships>"#.to_vec(),
        );

        let signature = relationship_graph_signature(&entries, WORD_FILE_NAME, "test");

        assert_eq!(signature.len(), 2);
        assert_eq!(signature[0].relationship_id.as_deref(), Some("rId1"));
        assert_eq!(signature[1].relationship_id.as_deref(), Some("rId2"));
        assert_eq!(signature[0].target, signature[1].target);
        assert_eq!(
            signature[0].resolved_target.as_deref(),
            Some("word/document.xml")
        );
    }

    #[test]
    fn relationship_graph_preserves_incomplete_records_for_fidelity_checks() {
        let entries = BTreeMap::from([(
            "xl/pivotCache/_rels/pivotCacheDefinition1.xml.rels".to_string(),
            br#"<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/externalLinkPath" TargetMode="External"/></Relationships>"#.to_vec(),
        )]);

        let signature = relationship_graph_signature(&entries, FILE_NAME, "test");

        assert_eq!(signature.len(), 1);
        assert_eq!(signature[0].relationship_id, None);
        assert_eq!(signature[0].target, None);
        assert_eq!(signature[0].resolved_target, None);
        assert_eq!(signature[0].target_mode, "External");
    }

    #[test]
    fn relationship_graph_is_independent_of_relationship_element_order() {
        let first = br#"<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://example.com/one" Target="../media/one.png"/><Relationship Id="rId2" Type="http://example.com/two" Target="../media/two.png"/></Relationships>"#;
        let second = br#"<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId2" Type="http://example.com/two" Target="../media/two.png"/><Relationship Id="rId1" Type="http://example.com/one" Target="../media/one.png"/></Relationships>"#;
        let first = BTreeMap::from([("word/_rels/document.xml.rels".to_string(), first.to_vec())]);
        let second =
            BTreeMap::from([("word/_rels/document.xml.rels".to_string(), second.to_vec())]);

        let first = relationship_graph_signature(&first, WORD_FILE_NAME, "first");
        let second = relationship_graph_signature(&second, WORD_FILE_NAME, "second");

        assert_eq!(first, second);
        assert_eq!(first[0].source, "word/document.xml");
        assert_eq!(first[0].resolved_target.as_deref(), Some("media/one.png"));
    }

    #[test]
    fn relationship_entry_paths_map_to_their_sources() {
        assert_eq!(
            relationship_source_path("_rels/.rels").as_deref(),
            Some("/")
        );
        assert_eq!(
            relationship_source_path("_rels/root.xml.rels").as_deref(),
            Some("root.xml")
        );
        assert_eq!(
            relationship_source_path("word/_rels/document.xml.rels").as_deref(),
            Some("word/document.xml")
        );
        assert_eq!(relationship_source_path("word/document.xml"), None);
    }

    #[test]
    fn chart_data_labels_schema_order_is_compatible() {
        let producer_order = br#"<c:dLbls xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:dLblPos val="ctr"/><c:leaderLines/><c:showVal val="1"/><c:spPr/><c:txPr/></c:dLbls>"#;
        let schema_order = br#"<c:dLbls xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:spPr/><c:txPr/><c:dLblPos val="ctr"/><c:showVal val="1"/><c:leaderLines/></c:dLbls>"#;

        assert!(
            xml_equivalence_errors(producer_order, schema_order, FILE_NAME, ENTRY_NAME).is_empty()
        );
        assert!(
            !strict_xml_equivalence_errors(producer_order, schema_order, FILE_NAME, ENTRY_NAME,)
                .is_empty()
        );
    }

    #[test]
    fn chart_data_labels_content_difference_is_not_compatible() {
        let original = br#"<c:dLbls xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:dLblPos val="ctr"/><c:showVal val="1"/><c:spPr/></c:dLbls>"#;
        let changed = br#"<c:dLbls xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:spPr/><c:dLblPos val="ctr"/><c:showVal val="0"/></c:dLbls>"#;

        assert!(!xml_equivalence_errors(original, changed, FILE_NAME, ENTRY_NAME).is_empty());
    }

    #[test]
    fn chart_data_labels_unknown_child_order_is_not_relaxed() {
        let original = br#"<c:dLbls xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:unknown/><c:spPr/><c:dLblPos val="ctr"/></c:dLbls>"#;
        let reordered = br#"<c:dLbls xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart"><c:spPr/><c:dLblPos val="ctr"/><c:unknown/></c:dLbls>"#;

        assert!(!xml_equivalence_errors(original, reordered, FILE_NAME, ENTRY_NAME).is_empty());
    }

    #[test]
    fn word_font_table_element_whitespace_is_compatible() {
        let indented = br#"<w:fonts xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">&#9;<w:font w:name="Example">&#9;&#9;<w:panose1 w:val="00000000000000000000"/>&#9;</w:font>&#9;</w:fonts>"#;
        let compact = br#"<w:fonts xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:font w:name="Example"><w:panose1 w:val="00000000000000000000"/></w:font></w:fonts>"#;

        assert!(
            xml_equivalence_errors(indented, compact, WORD_FILE_NAME, "word/fontTable.xml")
                .is_empty()
        );
    }

    #[test]
    fn word_font_table_non_xml_whitespace_is_not_compatible() {
        let with_nbsp = br#"<w:fonts xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">&#160;<w:font w:name="Example"/></w:fonts>"#;
        let compact = br#"<w:fonts xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:font w:name="Example"/></w:fonts>"#;

        assert!(
            !xml_equivalence_errors(with_nbsp, compact, WORD_FILE_NAME, "word/fontTable.xml")
                .is_empty()
        );
    }

    #[test]
    fn pandoc_ns0_reference_synthetic_sectpr_type_attr_is_exactly_relaxed() {
        let original = br#"<ns0:document xmlns:ns0="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><ns0:body><ns0:sectPr ns0:type="continuous"/></ns0:body></ns0:document>"#;
        let roundtripped = br#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:sectPr/></w:body></w:document>"#;

        assert!(
            xml_equivalence_errors(
                original,
                roundtripped,
                PANDOC_NS0_FILE_NAME,
                "word/document.xml",
            )
            .is_empty()
        );
        assert!(
            !strict_xml_equivalence_errors(
                original,
                roundtripped,
                PANDOC_NS0_FILE_NAME,
                "word/document.xml",
        ...
        assert!(
            !xml_equivalence_errors(original, roundtripped, WORD_FILE_NAME, "word/document.xml",)
                .is_empty()
        );

        let different_value = br#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:sectPr w:type="nextPage"/></w:body></w:document>"#;
        assert!(
            !xml_equivalence_errors(
                different_value,
                roundtripped,
    ...
    #[test]
    fn word_table_style_override_cell_properties_schema_order_is_compatible() {
        let producer_order = br#"<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:style w:type="table" w:styleId="Example"><w:tblStylePr w:type="firstRow"><w:tcPr><w:vAlign w:val="bottom"/><w:tcBorders><w:bottom w:val="single"/></w:tcBorders></w:tcPr></w:tblStylePr></w:style></w:styles>"#;
        let schema_order = br#"<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:style w:type="table" w:styleId="Example"><w:tblStylePr w:type="firstRow"><w:tcPr><w:tcBorders><w:bottom w:val="single"/></w:tcBorders><w:vAlign w:val="bottom"/></w:tcPr></w:tblStylePr></w:style></w:styles>"#;
    ...
    #[test]
    fn word_table_style_override_unknown_child_order_is_not_relaxed() {
        let original = br#"<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:style w:type="table" w:styleId="Example"><w:tblStylePr w:type="firstRow"><w:tcPr><w:unknown/><w:vAlign w:val="bottom"/><w:tcBorders/></w:tcPr></w:tblStylePr></w:style></w:styles>"#;
        let reordered = br#"<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:style w:type="table" w:styleId="Example"><w:tblStylePr w:type="firstRow"><w:tcPr><w:tcBorders/><w:vAlign w:val="bottom"/><w:unknown/></w:tcPr></w:tblStylePr></w:style></w:styles>"#;

        assert!(
            !xml_equivalence_errors(original, reordered, WORD_FILE_NAME, "word/styles.xml",)
                .is_empty()
        );
    }

    #[test]
    fn identical_word_settings_proof_state_duplicates_are_compatible() {
        let duplicated = br#"<w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:proofState w:spelling="clean" w:grammar="clean"/><w:rsids/><w:proofState w:spelling="clean" w:grammar="clean"/></w:settings>"#;
        let singleton = br#"<w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:proofState w:spelling="clean" w:grammar="clean"/><w:rsids/></w:settings>"#;

        assert!(
            xml_equivalence_errors(duplicated, singleton, WORD_FILE_NAME, "word/settings.xml",)
                .is_empty()
        );
    }

    #[test]
    fn different_word_settings_proof_state_duplicates_are_not_compatible() {
        let duplicated = br#"<w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:proofState w:spelling="clean" w:grammar="clean"/><w:rsids/><w:proofState w:spelling="dirty" w:grammar="clean"/></w:settings>"#;
        let singleton = br#"<w:settings xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:proofState w:spelling="clean" w:grammar="clean"/><w:rsids/></w:settings>"#;
    ...
    #[test]
    fn math_delimiter_properties_schema_order_is_compatible() {
        let producer_order = br#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><w:body><m:d><m:dPr><m:begChr m:val="("/><m:endChr m:val=")"/><m:sepChr m:val=""/><m:grow/></m:dPr></m:d></w:body></w:document>"#;
        let schema_order = br#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><w:body><m:d><m:dPr><m:begChr m:val="("/><m:sepChr m:val=""/><m:endChr m:val=")"/><m:grow/></m:dPr></m:d></w:body></w:document>"#;
    ...
    #[test]
    fn math_delimiter_properties_unknown_child_order_is_not_relaxed() {
        let original = br#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><w:body><m:d><m:dPr><m:unknown/><m:endChr/><m:sepChr/></m:dPr></m:d></w:body></w:document>"#;
        let reordered = br#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><w:body><m:d><m:dPr><m:sepChr/><m:endChr/><m:unknown/></m:dPr></m:d></w:body></w:document>"#;

        assert!(
            !xml_equivalence_errors(original, reordered, WORD_FILE_NAME, "word/document.xml",)
                .is_empty()
        );
    }
}
