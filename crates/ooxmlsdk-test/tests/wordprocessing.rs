...
#[cfg(not(feature = "mce"))]
use ooxmlsdk_test::{assert_xml_namespace_prefixes, xml_namespace_prefixes_match};
...
fn document_round_trip_writes_fixed_xml_declaration() {
    ...
    let (_parsed, serialized, _reparsed) = assert_stable_roundtrip::<Document>(xml);
...
#[test]
fn document_reads_inherited_namespace_alias_attributes() {
    let xml = r#"<ns0:document xmlns:ns0="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><ns0:body><ns0:p><ns0:r><ns0:t>ref</ns0:t></ns0:r></ns0:p><ns0:sectPr ns0:type="continuous"><ns0:pgSz ns0:w="12240" ns0:h="15840"/><ns0:pgMar ns0:top="567" ns0:right="567" ns0:bottom="567" ns0:left="567" ns0:header="567" ns0:footer="567" ns0:gutter="0"/></ns0:sectPr></ns0:body></ns0:document>"#;

    let document = Document::from_bytes(xml.as_bytes()).expect("namespace-aliased document");
    let section = first_body(&document)
        .section_properties
        .as_deref()
        .expect("section properties");
    let page_size = section.page_size.as_ref().expect("page size");
    let page_margin = section.page_margin.as_ref().expect("page margin");

    assert_eq!(
        page_size.width.as_ref().map(ToString::to_string).as_deref(),
        Some("12240")
    );
    assert_eq!(
        page_size
    ...
    assert_eq!(
        page_margin.top.as_ref().map(ToString::to_string).as_deref(),
        Some("567")
    );
    assert_eq!(
        page_margin
    ...
    assert!(
        section.section_type.is_none(),
        "a prefixed attribute must not match the unprefixed child QName"
    );
}

#[test]
fn document_reads_root_attribute_alias_declared_after_use() {
    let xml = r#"<w:document ns0:conformance="strict" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:ns0="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body/></w:document>"#;

    let (document, serialized, reparsed) = assert_stable_roundtrip::<Document>(xml);

    assert!(document.w_conformance.is_some());
    assert!(serialized.contains(r#" w:conformance="strict""#));
...
fn document_round_trip_with_trailing_whitespace_after_last_element() {
    let (parsed, serialized, reparsed) = assert_stable_roundtrip::<Document>(
        fixtures::WORDPROCESSING_DOCUMENT_TRAILING_WHITESPACE_XML,
    );
...
fn document_round_trip_preserves_static_mce_attributes_and_alternate_content() {
    ...
    assert_xml_namespace_prefixes(document.mc_ignorable.as_deref(), &["w14"]);
    assert_eq!(
...
fn document_preserves_known_extension_attribute_from_markup_compatibility_test() {
    ...
    let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" mc:Ignorable="w14" mc:PreserveAttributes="w14:noSpellErr"><w:body><w:p w14:noSpellErr="1"/></w:body></w:document>"#;
...
fn alternate_content_preserves_ignored_unknown_process_content_and_must_understand_metadata() {
    ...
    assert!(
        alternate_content.contains(
            r#"<mc:Choice Requires="uns1" mc:ProcessContent="*" mc:MustUnderstand="uns1">"#
        )
    );
    assert!(
        alternate_content
            .contains(r#"<uns1:e1uk1 uns1:a1uk1="attribute1 from unknown namespace1.">"#)
    );
    ...
    assert!(
        serialized.contains(r#"<uns1:e1uk1 uns1:a1uk1="attribute1 from unknown namespace1.">"#)
    );
}
...
fn body_alternate_content_without_selected_content_round_trips() {
    ...
    assert!(matches!(
        ...
        ooxmlsdk::schemas::schemas_openxmlformats_org_markup_compatibility_2006::AlternateContentChoice::Choice(choice)
            if xml_namespace_prefixes_match(&choice.requires, &["w13"])
    ));
    let w15_choice = alternate_content[2]
        ...
        .find_map(|choice| match choice {
            ooxmlsdk::schemas::schemas_openxmlformats_org_markup_compatibility_2006::AlternateContentChoice::Choice(choice)
                if xml_namespace_prefixes_match(&choice.requires, &["w15"]) =>
            {
                Some(choice.as_ref())
            }
            _ => None,
    ...
    assert_xml_namespace_prefixes(w15_choice.mc_must_understand.as_deref(), &["w15"]);
...
fn document_round_trip_preserves_rich_content_and_hyperlinks_from_openxml_asset() {
    ...
    let Some(SdtPropertiesChoice::SdtAlias(alias)) = properties.sdt_properties_choice.first()
    else {
