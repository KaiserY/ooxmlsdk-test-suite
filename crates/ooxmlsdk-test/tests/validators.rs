...
fn union_attribute_validation_test() {
    ...
    let mut element = Color {
        val: Some("auto".to_string()),
        ..Default::default()
    };
    assert!(element.validate().is_empty());

    element.val = Some("123456".to_string());
    assert!(element.validate().is_empty());

    element.val = Some("FF12AB".to_string());
    assert!(element.validate().is_empty());

    element.val = Some(String::new());
    assert!(!element.validate().is_empty());

    element.val = Some("auto1".to_string());
    assert!(!element.validate().is_empty());

    element.val = Some("1234567".to_string());
    assert!(!element.validate().is_empty());
...
fn any_uri_string_format_trims_edge_whitespace_like_upstream() {
    assert!(
        validate_string_format("AnyUriRestrictionTests", "text", &"", StringFormatKind::Uri)
            .is_ok()
    );
    assert!(
...
fn non_negative_integer_number_type_rejects_negative_values() {
    assert!(
        validate_number_type("NumberTypeTests", "value", &"0", "xsd:nonNegativeInteger").is_ok()
    );
    assert!(
