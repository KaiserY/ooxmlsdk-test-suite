...
fn media_data_part_reference_relationships_can_be_added_removed_and_reopened() {
    ...
    assert_eq!(
        reopened_slide2
            .data_part_reference_relationships(&reopened)
            .filter(
                |relationship| relationship.target_part_id() == reopened_media_data_part.part_id()
            )
            .count(),
        1
    );
...
fn add_data_part_reference_relationship_from_existing_reuses_id_type_and_target() {
    ...
    let reopened_header = reopened_main
        ...
        .find(|part| {
            reopened_main.get_id_of_part(&reopened, part) == Some("rIdSdkHeaderForDataRef")
        })
        .unwrap();
...
fn create_from_template_returns_editable_regular_document_packages() {
    ...
    let mut word =
        WordprocessingDocument::create_from_template(doc_sample("Document.dotx")).unwrap();
...
fn package_copy_helpers_round_trip_spreadsheet_and_presentation_documents() {
    ...
    let mut reopened_spreadsheet =
        SpreadsheetDocument::new(Cursor::new(spreadsheet_bytes)).unwrap();
    assert!(
