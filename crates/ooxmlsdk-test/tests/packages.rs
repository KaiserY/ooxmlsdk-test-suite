...
use ooxmlsdk::common::{
    MediaDataPart, ReferenceRelationshipKind, RelationshipRef, RelationshipTargetKind, SdkError,
};
use ooxmlsdk::parts::{
...
#[cfg(feature = "mce")]
use ooxmlsdk::schemas::schemas_openxmlformats_org_drawingml_2006_main::GraphicDataChoice;
#[cfg(feature = "mce")]
...
use ooxmlsdk_test::{assert_xml_namespace_prefixes, fixtures};
...
fn wordprocessing_package_with_duplicate_target_relationships() -> Cursor<Vec<u8>> {
    let mut buffer = Cursor::new(Vec::new());
    {
        let mut zip = zip::ZipWriter::new(&mut buffer);
        let options = zip::write::SimpleFileOptions::default();
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/></Relationships>"#,
        ...
        zip.write_all(
            br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rIdCommentsFirst" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments" Target="comments.xml"/><Relationship Id="rIdCommentsSecond" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments" Target="comments.xml"/></Relationships>"#,
        )
        .unwrap();
...
fn media_data_part_from_relationship<P: SdkPackage>(
    package: &P,
...
<pkg:package xmlns:pkg="http://schemas.microsoft.com/office/2006/xmlPackage">
  ...
  <pkg:part pkg:name="/word/document.xml" pkg:contentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml">
    ...
    let package = WordprocessingDocument::from_flat_opc_str(flat_opc).unwrap();
    let main_part = package.main_document_part().unwrap();
    let root = main_part.root_element(&package).unwrap();
...
fn wordprocessing_document_supports_eager_and_lazy_root_loading() {
    ...
    let lazy = WordprocessingDocument::new_from_file(&path).unwrap();
    let lazy_main = lazy.main_document_part().unwrap();
    assert!(!lazy_main.is_root_element_loaded(&lazy));
    let lazy_root = lazy_main.root_element(&lazy).unwrap();
...
fn process_loaded_parts_only_processes_lazy_root_element() {
    ...
    assert!(!main_part.is_root_element_loaded(&package));
    let serialized = main_part.root_element(&package).unwrap().to_xml().unwrap();
    ...
    let saved = package.to_package_bytes().unwrap();
    let saved_document = String::from_utf8(package_entry_data(saved, "word/document.xml")).unwrap();
    assert!(
        !saved_document.contains("AlternateContent"),
        "{saved_document}"
    );
    assert!(
        saved_document.contains("<w:t>choice</w:t>"),
        "{saved_document}"
    );
}
...
fn process_loaded_parts_only_replaces_presentation_shape_tree_choice() {
    ...
    let package = PresentationDocument::new_from_file_with_settings(path, settings).unwrap();
    ...
    let slide = slide_part.root_element(&package).unwrap();
...
#[cfg(feature = "mce")]
#[test]
fn process_all_parts_selects_typed_model3d_with_raster_preview() {
    let settings = OpenSettings {
        open_mode: PackageOpenMode::Lazy,
        markup_compatibility_process_settings: MarkupCompatibilityProcessSettings {
            process_mode: MarkupCompatibilityProcessMode::ProcessAllParts,
    ...
    let path = corpus_file(
        "corpus/Open-XML-SDK/test/DocumentFormat.OpenXml.Tests.Assets/assets/TestFiles/3dtestdash.pptx",
    );
    let package = PresentationDocument::new_from_file_with_settings(path, settings).unwrap();
    let presentation_part = package.presentation_part().unwrap();
    let slide_part = presentation_part.slide_parts(&package).next().unwrap();
    ...
    let model = slide
        ...
        .find_map(|choice| match choice {
            ShapeTreeChoice::GraphicFrame(frame) => frame
                .graphic
                .graphic_data
                .graphic_data_choice
                .iter()
                .find_map(|choice| match choice {
                    GraphicDataChoice::Model3D(model) => Some(model.as_ref()),
                    _ => None,
                }),
            _ => None,
        })
        .expect("Microsoft 365 MCE selection must expose typed am3d:model3d");
    let raster_blip = model
        .model3_d_raster
        .as_deref()
        .and_then(|raster| raster.blip.as_deref())
        .expect("model3d must retain its static raster preview");

    assert_eq!(raster_blip.embed.as_deref(), Some("rId3"));
    let serialized = slide.to_xml().unwrap();
    assert!(!serialized.contains("AlternateContent"), "{serialized}");
    assert!(serialized.contains("<am3d:model3d"), "{serialized}");
    assert!(!serialized.contains("<p:pic"), "{serialized}");
}
...
fn wordprocessing_mce_packages_open_save_and_reopen_from_autosave_tests() {
    ...
    for file_name in ["mcdoc.docx", "mcinleaf.docx"] {
        ...
        .unwrap();
        let main_part = package.main_document_part().unwrap();
        assert!(main_part.root_element(&package).is_ok());
        ...
        let reopened_main = reopened.main_document_part().unwrap();
        assert!(reopened_main.root_element(&reopened).is_ok());
    }
}
...
fn open_settings_default_markup_compatibility_matches_upstream() {
    ...
    let presentation =
        PresentationDocument::new_from_file(doc_sample("Presentation.pptx")).unwrap();
...
fn wordprocessing_sdt_alias_mutation_is_saved_from_mc_support_test() {
    ...
    let reopened_main = reopened.main_document_part().unwrap();
    let reopened_root = reopened_main.root_element(&reopened).unwrap();
...
fn wordprocessing_font_table_touch_preserves_w14_namespace_from_mc_support_test() {
    ...
    let fonts = font_table_part.root_element(&package).unwrap();
    assert_xml_namespace_prefixes(fonts.mc_ignorable.as_deref(), &["w14"]);
    ...
    assert!(
        font_table_xml
            .contains(r#"xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml""#)
    );
...
fn package_relationships_resolve_with_container_local_part_factory() {
    ...
    let resolved = package
        .get_part_by_id(main_part_id)
        .and_then(|part| part_ref_variant!(part, MainDocumentPart))
        .unwrap();

    assert_eq!(resolved, main_part);
    ...
    assert!(package.get_part_by_id("invalidId").is_none());
}
...
fn package_relationship_helpers_match_openxml_container_api_shape() {
    ...
    .unwrap();
    let main_part = package.main_document_part().unwrap();
    let main_part_id = package.get_id_of_part(&main_part).unwrap().to_string();
    assert!(package.get_id_of_part(&main_part).is_ok());
    assert!(
        package
            .get_parts_of_type::<MainDocumentPart>()
            .next()
    ...
    assert!(matches!(
        package.try_get_part_by_id(&main_part_id).unwrap(),
        PartRef::MainDocumentPart(_)
    ));
    assert!(matches!(
        package.try_get_part_by_id("rIdMissing"),
        Err(SdkError::PartRelationshipNotFound { .. })
    ));
...
fn package_save_writes_package_relationships_from_parts() {
    let package = WordprocessingDocument::new_from_file(doc_sample("Of16-01.docx")).unwrap();
    let main_part = package.main_document_part().unwrap();
    let main_relationship_id = package.get_id_of_part(&main_part).unwrap().to_string();
    assert!(
        package
            .parts()
            .any(|entry| entry.part == PartRef::MainDocumentPart(main_part.clone()))
    );
    ...
    let reopened_main = reopened.main_document_part().unwrap();
    assert_eq!(
        reopened.get_id_of_part(&reopened_main).unwrap(),
        main_relationship_id
    );
    assert_ne!(reopened_main, main_part);
}
...
fn package_accessors_read_live_relationship_storage_after_mutation() {
    ...
    assert_eq!(package.core_file_properties_part().unwrap(), core);
    ...
    assert_eq!(
        package.core_file_properties_part().unwrap(),
        replacement_core
    );
    ...
    assert_eq!(package.main_document_part().unwrap(), replacement_main);
}
...
fn wordprocessing_child_accessors_are_relationship_backed_handles() {
    ...
    assert!(main_part.get_id_of_part(&package, &styles_part).is_ok());
    assert_eq!(
        main_part.style_definitions_part(&package).unwrap(),
        styles_part
    );
    assert_eq!(
        main_part
...
fn wordprocessing_root_element_access_matches_openxml_part_root_element_test() {
    ...
    let package = WordprocessingDocument::new_from_file(doc_sample("complex0.docx")).unwrap();
    let main_part = package.main_document_part().unwrap();
    ...
    assert!(main_part.root_element(&package).is_ok());
    assert!(comments_part.root_element(&package).is_ok());
    assert!(!image_part.data(&package).unwrap().is_empty());
    assert_eq!(
        main_part.get_id_of_part(&package, &comments_part).unwrap(),
        "rId7"
    );
    let resolved_comments = main_part
        .get_part_by_id(&package, "rId7")
        .and_then(|part| part_ref_variant!(part, WordprocessingCommentsPart))
        .unwrap();
    assert_eq!(resolved_comments, comments_part);
    assert_eq!(
        main_part
...
fn part_root_element_rejects_whitespace_only_xml() {
    ...
    assert!(main_part.root_element(&package).is_err());
...
fn part_relationship_ids_can_change_for_child_parts() {
    ...
    assert_eq!(
        reopened_main
            .get_id_of_part(&reopened, &reopened_comments)
            .unwrap(),
        "rIdSdkComments"
    );
    assert!(
...
#[test]
fn duplicate_target_relationships_preserve_each_id_and_require_explicit_mutation() {
    let package =
        WordprocessingDocument::new(wordprocessing_package_with_duplicate_target_relationships())
            .unwrap();
    let main = package.main_document_part().unwrap();
    let related = main
        .related_parts_of_type::<_, WordprocessingCommentsPart>(&package)
    ...
    assert_eq!(
        related
            ...
            .map(|(id, _)| id.as_str())
            .collect::<Vec<_>>(),
        ["rIdCommentsFirst", "rIdCommentsSecond"]
    );
    assert_eq!(related[0].1, related[1].1);
    assert_eq!(
        main.get_id_of_part(&package, &related[0].1).unwrap(),
        "rIdCommentsFirst"
    );
    let handles = std::collections::HashSet::from([related[0].1.clone()]);
    assert!(handles.contains(&related[1].1));
    let part_refs = std::collections::HashSet::from([main
        .get_part_by_id(&package, "rIdCommentsFirst")
        .unwrap()]);
    assert!(part_refs.contains(&main.get_part_by_id(&package, "rIdCommentsSecond").unwrap()));

    let saved = package.to_package_bytes().unwrap();
    let mut package = WordprocessingDocument::new(Cursor::new(saved)).unwrap();
    let main = package.main_document_part().unwrap();
    let comments = main
        .related_parts_of_type::<_, WordprocessingCommentsPart>(&package)
        .next()
        ...
        .into_part();
    assert!(matches!(
        main.change_id_of_part(&mut package, &comments, "rIdReplacement"),
        Err(SdkError::AmbiguousPartRelationship {
            relationship_count: 2
        })
    ));

    assert!(main.delete_part(&mut package, comments.clone()).unwrap());
    assert!(main.get_part_by_id(&package, "rIdCommentsFirst").is_none());
    assert_eq!(
        main.get_id_of_part(&package, &comments).unwrap(),
        "rIdCommentsSecond"
    );
    assert!(main.get_part_by_id(&package, "rIdCommentsSecond").is_some());
    assert!(
        main.delete_part_by_id(&mut package, "rIdCommentsSecond")
            .unwrap()
    );
    assert!(main.get_part_by_id(&package, "rIdCommentsSecond").is_none());
}
...
fn delete_invalid_child_part_id_is_safe() {
    ...
    assert!(main_part.get_part_by_id(&package, "invalidId").is_none());
    assert!(
        !main_part
...
fn part_get_all_parts_and_parent_parts_follow_reachable_relationship_graph() {
    ...
    let image_part = header_part
        ...
        .unwrap();
    assert!(main_part.get_id_of_part(&package, &header_part).is_ok());
    assert!(main_part.get_id_of_part(&package, &settings_part).is_ok());
    assert!(header_part.get_id_of_part(&package, &image_part).is_ok());
    assert!(matches!(
        main_part.get_id_of_part(&package, &image_part),
        Err(SdkError::PartNotReferenced)
    ));
    assert!(
        main_part
    ...
    let main_descendants: Vec<_> = main_part.get_all_parts(&package).collect();
    assert!(main_descendants.contains(&PartRef::HeaderPart(header_part.clone())));
    assert!(main_descendants.contains(&PartRef::DocumentSettingsPart(settings_part.clone())));
    assert!(main_descendants.contains(&PartRef::ImagePart(image_part.clone())));
    assert!(!main_descendants.contains(&PartRef::MainDocumentPart(main_part.clone())));

    let image_parents: Vec<_> = image_part.get_parent_parts(&package).collect();
    assert_eq!(
        image_parents,
        vec![PartRef::HeaderPart(header_part.clone())]
    );

    let header_parents: Vec<_> = header_part.get_parent_parts(&package).collect();
    assert_eq!(
        header_parents,
        vec![PartRef::MainDocumentPart(main_part.clone())]
    );
}
...
fn media_reference_relationships_resolve_shared_data_part_from_openxml_package_test() {
    ...
    assert!(
        slides
            .iter()
            .all(|slide| presentation_part.get_id_of_part(&package, slide).is_ok())
    );
    ...
    for slide in slides {
        for relationship in slide.data_part_reference_relationships(&package) {
            if relationship
                .targets_media_data_part(&package, &media_part)
                .unwrap()
            {
...
fn media_data_part_reference_relationships_can_be_added_removed_and_reopened() {
    ...
    let media_bytes = media_data_part.try_data_bytes(&package).unwrap();
    let media_bytes_again = media_data_part.try_data_bytes(&package).unwrap();
    assert!(!media_bytes.is_empty());
    assert_eq!(media_bytes.as_ptr(), media_bytes_again.as_ptr());
    assert_eq!(
        media_data_part
            .data_part_reference_relationships(&package)
            .count(),
    ...
    let slide1_media_relationships: Vec<_> = slide1_relationships
        ...
        .filter(|relationship| {
            relationship
                .targets_media_data_part(&package, &media_data_part)
                .unwrap()
        })
        .collect();
    ...
    let reopened_slide1_relationships: Vec<_> = reopened_slide1
        ...
        .filter(|relationship| {
            relationship
                .targets_media_data_part(&reopened, &reopened_media_data_part)
                .unwrap()
        })
        .collect();
    ...
    assert!(
        reopened_slide1_relationships[0]
            .targets_media_data_part(&reopened, &reopened_media_data_part)
            .unwrap()
    );
    assert_eq!(
...
fn create_media_data_parts_adds_data_part_reference_relationships_and_saves() {
    ...
    assert!(
        audio_relationship
            .targets_media_data_part(&package, &wav)
            .unwrap()
    );
    assert_eq!(
    ...
    assert!(
        media_relationship
            .targets_media_data_part(&package, &avi)
            .unwrap()
    );
    assert_eq!(
    ...
    let reopened_audio_relationship = reopened_relationships
        ...
        .find(|relationship| relationship.id() == "rIdSdkAudio")
        .unwrap();
    let reopened_audio_part =
        media_data_part_from_relationship(&reopened, reopened_audio_relationship);
    assert_eq!(
    ...
    let reopened_media_part =
        media_data_part_from_relationship(&reopened, reopened_media_relationship);
    assert_eq!(
    ...
    let reopened_video_part =
        media_data_part_from_relationship(&reopened, reopened_video_relationship);
    assert!(reopened_video_part.is_same_part(&reopened_media_part));
}
...
fn add_data_part_reference_relationship_from_existing_reuses_id_type_and_target() {
    ...
    let reopened_header = reopened_main
        ...
        .find(|part| {
            reopened_main
                .get_id_of_part(&reopened, part)
                .is_ok_and(|id| id == "rIdSdkHeaderForDataRef")
        })
        .unwrap();
    ...
    let reopened_media_part = media_data_part_from_relationship(&reopened, &reopened_relationship);
    assert_eq!(
        reopened_media_part.content_type(&reopened),
        Some("audio/mp3")
    );
...
fn wordprocessing_hyperlink_relationships_are_preserved_from_openxml_part_test() {
    ...
    assert!(rid15.target_part(&package).unwrap().is_none());
    ...
    assert!(rid18.target_part(&package).unwrap().is_none());
}
...
fn set_root_element_replaces_lazy_root_and_is_saved() {
    ...
    assert_eq!(
        main_document_body_child_count(main_part.root_element(&package).unwrap()),
        0
    );
...
fn add_new_header_part_creates_relationship_content_type_and_root_slot() {
    ...
    let header_part = main_part
        ...
        .unwrap();
    assert_eq!(
        main_part.get_id_of_part(&package, &header_part).unwrap(),
        relationship_id
    );
    assert_eq!(
    ...
    assert!(reopened_header.root_element(&reopened).is_ok());
}
...
fn generated_child_part_constraints_reject_invalid_singleton_and_parent_combinations() {
    ...
    let settings = main_part
        ...
        .unwrap();
    assert!(main_part.get_id_of_part(&package, &settings).is_ok());
    assert!(
        main_part
            .add_new_part_auto_id::<_, DocumentSettingsPart>(&mut package)
            .is_err()
...
fn package_add_new_part_creates_package_relationship() {
    ...
    let ribbon_part = package
        ...
        .unwrap();
    assert_eq!(
        package.get_id_of_part(&ribbon_part).unwrap(),
        relationship_id
    );
...
fn add_new_part_with_content_type_and_path_uses_specified_package_path() {
    ...
    assert_eq!(
        main_part.get_id_of_part(&package, &image_part).unwrap(),
        relationship_id
    );
...
fn part_data_helpers_read_text_and_write_bytes() {
    ...
    let shared_data = custom_xml.try_data_bytes(&package).unwrap();
    let shared_data_again = custom_xml.try_data_bytes(&package).unwrap();
    assert_eq!(shared_data.as_ref(), xml.as_bytes());
    assert_eq!(shared_data.as_ptr(), shared_data_again.as_ptr());
    assert_eq!(
...
fn add_alternative_format_import_part_with_id_feeds_data_and_saves() {
    ...
    assert_eq!(
        main_part.get_id_of_part(&package, &alt_chunk).unwrap(),
        relationship_id
    );
...
fn add_custom_xml_part_with_id_uses_content_type_and_relationship_id() {
    ...
    assert_eq!(
        main_part.get_id_of_part(&package, &custom_xml).unwrap(),
        relationship_id
    );
    assert_eq!(
...
fn generic_add_new_part_with_content_type_and_extension_saves_custom_extension() {
    ...
    assert_eq!(
        main_part.get_id_of_part(&package, &part).unwrap(),
        relationship_id
    );
...
fn add_thumbnail_part_with_id_uses_content_type_and_relationship_id() {
    ...
    assert_eq!(package.get_id_of_part(&thumbnail).unwrap(), relationship_id);
...
fn add_extended_part_with_id_supports_package_part_and_nested_extended_parts() {
    ...
    assert_eq!(
        package.get_id_of_part(&package_extended).unwrap(),
        "rIdSdkPackageExtended"
    );
    assert_eq!(
        main_part.get_id_of_part(&package, &part_extended).unwrap(),
        "rIdSdkMainExtended"
    );
    assert_eq!(
        part_extended
            .get_id_of_part(&package, &nested_extended)
            .unwrap(),
        "rIdSdkNestedExtended"
    );
...
fn extended_part_relationship_cycle_uses_storage_edges_without_recursive_handles() {
    ...
    assert_eq!(reopened_back, reopened_first);
...
fn add_part_and_create_relationship_to_part_share_existing_parts() {
    ...
    assert_eq!(
        extended.get_id_of_part(&package, &image).unwrap(),
        "rIdSharedImage"
    );
...
fn create_relationship_to_part_reuses_existing_parts_from_package() {
    ...
    let slide1_relationship_id = slide1
        ...
        .unwrap();
    let attached_slide_layout2 = slide1
        .get_part_by_id(&package, &slide1_relationship_id)
        .and_then(|part| part_ref_variant!(part, SlideLayoutPart))
        .unwrap();
    assert_eq!(attached_slide_layout2, slide_layout2);
    ...
    let attached_slide_layout1 = slide2
        .get_part_by_id(&package, "rId1001")
        .and_then(|part| part_ref_variant!(part, SlideLayoutPart))
        .unwrap();
    assert_eq!(attached_slide_layout1, slide_layout1);
...
fn add_part_from_package_imports_part_tree_relationships_and_data_parts() {
    ...
    let imported_media_part = media_data_part_from_relationship(&target, &data_refs[0]);
...
fn add_main_document_part_creates_fixed_main_part_path() {
    ...
    assert_eq!(
        main_document_body_child_count(reopened_main.root_element(&reopened).unwrap()),
        0
    );
}
...
fn create_apis_create_office_document_packages() {
    ...
    let reopened_word = WordprocessingDocument::new(Cursor::new(word_buffer.into_inner())).unwrap();
    let reopened_word_main = reopened_word.main_document_part().unwrap();
    let reopened_word_root = reopened_word_main.root_element(&reopened_word).unwrap();
    ...
    let reopened_workbook = reopened_workbook_part
        .root_element(&reopened_spreadsheet)
        .unwrap();
    ...
    let reopened_presentation_root = reopened_presentation_part
        .root_element(&reopened_presentation)
        .unwrap();
    assert_eq!(
...
fn create_from_template_returns_editable_regular_document_packages() {
    ...
    let word = WordprocessingDocument::create_from_template(doc_sample("Document.dotx")).unwrap();
    assert_eq!(word.document_type(), WordprocessingDocumentType::Document);
    let word_main = word.main_document_part().unwrap();
    assert!(word_main.root_element(&word).is_ok());
    ...
    .unwrap();
    assert!(word_content_types.contains(
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"
    ));
    assert!(!word_content_types.contains(
        "application/vnd.openxmlformats-officedocument.wordprocessingml.template.main+xml"
    ...
    assert!(workbook_part.root_element(&spreadsheet).is_ok());
    ...
    assert!(
        !spreadsheet_content_types.contains(
            "application/vnd.openxmlformats-officedocument.spreadsheetml.template.main+xml"
        )
    );
    ...
    assert!(presentation_part.root_element(&presentation).is_ok());
...
fn document_type_is_read_from_existing_template_packages() {
    ...
    let presentation =
        PresentationDocument::new_from_file(doc_sample("Presentation.potx")).unwrap();
    assert_eq!(
        presentation.document_type(),
        PresentationDocumentType::Template
    );
...
fn change_document_type_updates_main_part_content_type_and_preserves_relationships() {
    ...
    let content_types_xml =
        String::from_utf8(package_entry_data(bytes.clone(), "[Content_Types].xml")).unwrap();
    assert!(content_types_xml.contains(
        "application/vnd.openxmlformats-officedocument.wordprocessingml.template.main+xml"
    ));
    assert!(!content_types_xml.contains(
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"
...
fn package_save_roundtrips_unmodified_document() {
    ...
    let root = main_part.root_element(&roundtripped).unwrap();
...
#[test]
fn lazy_save_serializes_an_immutably_loaded_root() {
    let source = std::fs::read(doc_sample("Document.docx")).unwrap();
    let package = WordprocessingDocument::new_with_settings(
    ...
    .unwrap();
    let main_part = package.main_document_part().unwrap();
    let serialized_root = main_part.root_element(&package).unwrap().to_xml().unwrap();
...
#[test]
fn lazy_save_serializes_a_mutably_borrowed_dirty_root() {
    let source = std::fs::read(doc_sample("Document.docx")).unwrap();
    let mut package = WordprocessingDocument::new_with_settings(
        Cursor::new(source.clone()),
        lazy_open_settings(),
    ...
    main_part
        ...
        .unwrap()
        .body_choice
        .push(BodyChoice::Paragraph(Box::default()));
...
fn lazy_file_package_can_save_back_to_its_source_path() {
    ...
    let reopened = WordprocessingDocument::new_from_file(&path).unwrap();
    let main_part = reopened.main_document_part().unwrap();
    assert!(main_part.root_element(&reopened).is_ok());
...
fn package_copy_helpers_include_unsaved_root_changes() {
    ...
    let owned_copy = package.to_owned_package().unwrap();
    let copied_main = owned_copy.main_document_part().unwrap();
    assert!(
        copied_main
            .root_element(&owned_copy)
            .unwrap()
            .body
            .is_some()
    );
    let copied_header = copied_main
        ...
        .and_then(|part| part_ref_variant!(part, HeaderPart))
        .unwrap();
    assert!(copied_header.root_element(&owned_copy).is_ok());

    let reopened_from_bytes = WordprocessingDocument::new(Cursor::new(copied_bytes)).unwrap();
    let reopened_main = reopened_from_bytes.main_document_part().unwrap();
    assert!(
        reopened_main
            .root_element(&reopened_from_bytes)
            .unwrap()
            .body
            .is_some()
    );
...
fn spreadsheet_save_writes_current_state_without_later_mutations() {
    ...
    let reopened = SpreadsheetDocument::new(Cursor::new(saved)).unwrap();
    let reopened_workbook_part = reopened.workbook_part().unwrap();
    let reopened_root = reopened_workbook_part.root_element(&reopened).unwrap();
...
fn package_copy_helpers_round_trip_spreadsheet_and_presentation_documents() {
    ...
    let spreadsheet_copy = spreadsheet.to_owned_package().unwrap();
    assert_eq!(
    ...
    let presentation_copy = presentation.to_owned_package().unwrap();
    assert_eq!(
    ...
    assert!(presentation_part.root_element(&presentation_copy).is_ok());
    let reopened_presentation = PresentationDocument::new(Cursor::new(presentation_bytes)).unwrap();
    assert!(
...
fn package_save_as_file_round_trips_office_document_types() {
    ...
    //   CanSaveAsExcel
    //   CanSaveAsPowerpoint
    let word = WordprocessingDocument::new_from_file_with_settings(
        doc_sample("Document.docx"),
        lazy_open_settings(),
    )
    .unwrap();
