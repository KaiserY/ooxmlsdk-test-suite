use ooxmlsdk_pdf_test::{
    PdfBounds, PdfSummary, assert_pdf_rect_close, libreoffice_fixture, parse_pdf_rect,
    pdf_summary_for_fixture, raw_image_pixel_for_fixture, rendered_page_image_for_fixture,
};

fn fixture(name: &str) -> std::path::PathBuf {
    libreoffice_fixture(name)
}
...
fn assert_page_has_exact_text_segment(summary: &PdfSummary, page_index: usize, expected: &str) {
    let normalized_expected = pdf_text_content_key(expected);
    assert!(
        summary.text_segments.iter().any(|segment| {
            segment.page_index == page_index
                && pdf_text_content_key(&segment.text) == normalized_expected
        }),
        "missing exact page {page_index} text segment {expected:?}; text_segments={:?}",
...
fn assert_image_centers_are_not_grayscale(
    ...
    for bounds in image_bounds {
        let [r, g, b, _] = rendered
            ...
            .unwrap_or_else(|| {
                panic!("missing rendered image-center sample for bounds {bounds:?}")
            });
        assert!(
            !(r == g && r == b),
...
fn assert_text_origin_from_page_top_close(
    ...
    let media_box = parse_pdf_rect(&summary.media_boxes[page_index]).unwrap();
    let origin_y = text_segment_first_origin_y(summary, page_index, text);
...
fn text_segment_first_origin_y(summary: &PdfSummary, page_index: usize, text: &str) -> f32 {
...
fn assert_text_segment_wider_than_tall(summary: &PdfSummary, page_index: usize, text: &str) {
    let bounds = text_segment_bounds_on_page(summary, page_index, text);
    assert!(
        bounds.width() > bounds.height(),
        "page {page_index} text {text:?} should be horizontally oriented; bounds={bounds:?}"
    );
}
...
fn mapped_fixture_tdf147646_preserves_merged_cell_numbering() {
    ...
    assert_eq!(normalized_page_text(&summary, 0), "1. 2. 3. 4. 5. 6.");
}
...
fn mapped_fixture_tdf95495_preserves_numbering_level_visible_text() {
    let summary = render_summary("sw/qa/extras/ooxmlexport/data/tdf95495.docx");
    // The header has adjacent STYLEREF fields: `\n` supplies only "A",
    // while the plain field supplies only the heading text. The body remains
    // the counterexample proving that hierarchical numbering stays enabled.
    assert_page_starts_with(&summary, 0, "Appendix A Comment-Based Help");
    assert_page_not_contains(&summary, 0, "Appendix A A. Comment-Based Help");
    assert_page_contains(&summary, 0, "A.2.1");
    assert_page_contains(&summary, 0, ".DESCRIPTION");
    let label = exact_text_segment_bounds_on_page(&summary, 0, "A.1");
    let body = exact_text_segment_bounds_on_page(&summary, 0, "Introduction");
    let follow_space = body.left - label.right;
    // PdfSummary reports glyph-ink bounds, so this includes the label's
    // trailing side bearing and the body's leading side bearing around the
    // rendered space. The old hanging-indent path measured about 12.9pt.
    assert!(
        (follow_space - 6.10).abs() <= 0.25,
        "w:suff=space must continue after one rendered space, not at the hanging indent: label={label:?}, body={body:?}, gap={follow_space}"
    );
}
...
fn mapped_fixture_tdf117923_preserves_numbering_portion_text_and_height() {
    let summary = render_summary("sw/qa/extras/uiwriter/data/tdf117923.docx");
...
#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport.cxx:testShapeInFloattable
fn mapped_fixture_shape_in_floattable_keeps_inline_group_text_inside_host_row() {
    let summary = render_summary("sw/qa/extras/ooxmlexport/data/shape-in-floattable.docx");
    assert_eq!(summary.page_count, 1);
    assert_page_has_exact_text_segment(&summary, 0, "Hello");
    // The one-child WPG paints at its mapped child coordinates, while its
    // enclosing wp:inline extent owns the table-row line box.
    assert_text_top_from_page_top_close(&summary, 0, "Hello", 135.0, 1.0);
}
...
fn mapped_fixture_tdf124600_aligns_header_shape_text_with_body_text() {
    let summary = render_summary("sw/qa/extras/ooxmlimport/data/tdf124600.docx");
...
fn mapped_fixture_tdf136841_preserves_cropped_bitmap_dimensions() {
    ...
    // LibreOffice's import test observes a 76x76 screen bitmap. Word fixed
    // output instead rasterizes the 56.8pt EMF frame at 200 DPI while
    // preserving the Windows center color.
    assert_raw_image_source_pixel_color(
        "tdf136841.docx",
        &summary,
        157,
        157,
...
fn mapped_fixture_tdf113183_keeps_second_shape_inside_page_bounds() {
    ...
    // The LO assertion covers the layout frame. PDF path bounds additionally
    // include half of the authored outline width.
    assert_rightmost_path_within_page_right(&summary, 0, 1.5);
}
...
fn mapped_fixture_text_box_word_wrap_keeps_textbox_to_one_line_height() {
    ...
    assert_page_contains(&summary, 0, "Interní / Internal");
...
fn mapped_fixture_tdf133070_has_footer_preserves_relative_anchor_height() {
    let summary =
        render_summary("tdf133070_testRelativeAnchorHeightFromBottomMarginHasFooter.docx");
...
fn mapped_fixture_tdf123324_has_header_preserves_relative_anchor_height() {
    ...
    assert_path_geometry_height_close(&summary, 0, 127.55);
}
...
fn mapped_fixture_tdf165354_preserves_bottom_of_page_hyphenation_flow() {
    ...
    // The Office fixed PDF breaks "atmosphere" at the page boundary. PDFium
    // keeps that cross-page line-end hyphen as U+002D and starts page two with
    // the resumed suffix (same-page automatic hyphens extract as U+0002).
    assert_page_contains(&summary, 0, "except that it has an at-");
    assert_page_contains(&summary, 1, "mosphere. The Earth");
}
...
fn mapped_fixture_tdf88496_keeps_repeating_header_fallback_to_three_pages() {
    let summary = render_summary("sw/qa/extras/layout/data/tdf88496.docx");
    assert_eq!(summary.page_count, 3);
}
...
fn mapped_fixture_tdf109137_keeps_rectangle_on_first_page() {
...
fn mapped_fixture_tdf115630_keeps_inner_chart_area_width() {
    ...
    // Office fixed output uses a wider plot band than Writer's metafile.
    assert_path_width_between(&summary, 0, 280.0, 290.0);
}
...
fn mapped_fixture_tdf126244_preserves_vertical_multilevel_axis_labels() {
    ...
    // LibreOffice's source assertion explicitly requires the first level to
    // be horizontal; only its second and third levels are vertical.
    assert_text_segment_wider_than_tall(&summary, 0, "FIRST LEVEL");
}
...
fn mapped_fixture_tdf123651_keeps_shape_above_second_lorem_ipsum() {
    ...
    assert_path_top_from_page_top_close_with_tolerance(&summary, 0, 368.75, 2.0);
}
...
fn mapped_fixture_tdf129054_preserves_pie_chart_diameter() {
    ...
    // Writer's metafile uses a 230.75pt diameter; Word's fixed output uses
    // the smaller automatic plot region recorded by the Office golden.
    assert_path_height_close(&summary, 0, 201.8);
}
...
fn mapped_fixture_tdf137116_keeps_second_label_outside_pie_slice() {
    ...
    assert_text_left_difference_close(&summary, "datalabel2", "datalabel4", 25.7, 5.0);
}
...
fn mapped_fixture_tdf130031_keeps_data_label_below_data_point() {
    ...
    assert_text_top_from_page_top_close(&summary, 0, "23", 224.45, 2.5);
}
...
fn mapped_fixture_tdf130380_keeps_area_chart_from_shrinking() {
    ...
    // Preserve the Office plot-band height; the LO assertion's metafile Y
    // coordinate is not a PDF page coordinate.
    assert_path_height_close(&summary, 0, 196.35);
}
...
fn mapped_fixture_tdf134235_keeps_long_chart_title_inside_chart_area() {
    ...
    assert_page_text_segment_count(&summary, 0, 13);
}
...
fn mapped_fixture_tdf134146_breaks_long_y_axis_title_to_multiple_lines() {
    ...
    assert_page_contains(&summary, 0, "Horizontal, long axis title which breaks");
}
...
#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport12.cxx:testTdf117137
fn mapped_fixture_tdf117137_keeps_effect_marked_numbering_on_the_body_text_baseline() {
    let summary = render_summary("tdf117137.docx");
    let bounds = |text: &str| {
        summary
            .text_segments
            .iter()
            .find(|segment| segment.page_index == 0 && normalize_space(&segment.text) == text)
            .map(|segment| parse_pdf_rect(&segment.bounds).unwrap())
            .unwrap_or_else(|| panic!("missing page 0 text segment {text:?}"))
    };
    let label = bounds("1.");
    let body = bounds("Test.");
    assert!(
        (label.bottom - body.bottom).abs() <= 1.0,
        "numbering and body text must share a baseline: label={label:?}, body={body:?}"
    );
}

#[test]
// Source: ../core/sw/qa/core/layout/tabfrm.cxx:testInlineTableThenSplitFly
fn mapped_fixture_floattable_not_wrapped_by_table_keeps_cell_text_beside_vml_shape() {
    let summary = render_summary("floattable-not-wrapped-by-table.docx");
    assert_page_contains_in_order(&summary, 0, &["Before", "Left, A1", "Right, A1", "after"]);
}
...
fn mapped_fixture_tdf119908_keeps_exceeding_line_portion_visible_for_shrinking() {
    let summary = render_summary("sw/qa/extras/layout/data/tdf130088.docx");
    assert_page_contains(
        &summary,
        0,
...
fn mapped_fixture_tdf158333_keeps_expected_shrunk_line_portions() {
    ...
    // tdf#158776 is the negative boundary: the second guess must not pull
    // "cursus" onto this line.
    assert_page_has_exact_text_segment(
        &summary,
        0,
        "venenatis, quis commodo dolor posuere. Curabitur dignissim sapien quis ",
    );
    // The following line exercises the opposite boundary: Writer's one
    // shrinking guess may cross more than one UAX #14 segment, so "eget."
    // remains on the line after "molestie".
    assert_page_has_exact_text_segment(
        &summary,
        0,
        "cursus egestas. Donec blandit auctor arcu, nec pellentesque eros molestie eget. ",
    );
    // Office 16 keeps "Vestibulum" on the following line: fitting it here
    // would require shrinking twelve inter-word spaces to roughly 75.5%.
    // The positive "tempor" assertion above needs only roughly 81.6%, so the
    // pair pins both sides of Word's 20% smart-justify shrink boundary.
    assert_page_has_exact_text_segment(
        &summary,
        0,
        "Praesent vitae lacus vel leo sodales pharetra a a nibh. ",
    );
}
...
#[test]
// Sources:
// - ../core/sw/qa/writerfilter/dmapper/DomainMapper.cxx:testFrameprPagebreakStyle
// - ../core commit 48267890aa95 (tdf#168916), which transfers an effective
//   style-inherited page break from a framed paragraph to its outer anchor.
// ECMA-376 Part 1 sections 17.3.1.11 and 17.3.1.23 define framePr ownership
// and pageBreakBefore inheritance independently; neither cancels the other.
fn mapped_fixture_framepr_pagebreak_style_moves_the_frame_to_page_two() {
    let summary = render_summary("framepr-pagebreak-style.docx");

    assert_eq!(summary.page_count, 2);
    assert_page_contains(&summary, 0, "page 1");
    assert_page_not_contains(&summary, 0, "page 2 start");
    assert_page_not_contains(&summary, 0, "page 2 end");
    assert_page_contains_in_order(&summary, 1, &["page 2 start", "page 2 end"]);
    assert_page_not_contains(&summary, 1, "page 1");
}
...
fn mapped_fixture_tdf105143_keeps_shape_at_imported_vertical_position() {
    ...
    // The upstream assertion measures Writer's anchored-object frame. PDF
    // exposes only the inked VML path, whose authored path does not touch the
    // frame edge. Keep a narrow frame-to-ink allowance; the exact Office
    // golden covers the visible path position.
    assert_path_top_from_page_top_close_with_tolerance(&summary, 0, 6731.0 / 20.0, 1.5);
}
...
fn mapped_fixture_tdf60351_preserves_contour_polygon_shape() {
    ...
    // The six-point contour is wrap metadata, not a painted PDF polygon.
...
fn mapped_fixture_tdf97618_vml_shape_text_wrap_keeps_shape_width() {
    ...
    // LibreOffice currently disables its VML fallback assertion because this
    // fixture imports the DrawingML shape. Check the authored geometry rather
    // than the stroked PDF bounds.
    assert_path_geometry_width_close(&summary, 0, 2500.0 / 20.0);
}
...
fn mapped_fixture_gridbefore_keeps_grid_before_cell_position() {
    ...
    let line_step = (text_segment_first_origin_y(&summary, 0, "A3")
        - text_segment_first_origin_y(&summary, 0, "B1"))
    .abs();
    assert!(
        (line_step - 17.78).abs() <= 0.5,
        "legacy Chinese table-cell paragraph marks should keep the 17.78pt Word line step, got {line_step}pt"
    );
}

#[test]
// Sources: ../core/sw/qa/extras/ooxmlexport/ooxmlexport16.cxx:testTdf135773_numberingShading
// and ../core/sw/qa/extras/ooxmlexport/ooxmlexport15.cxx:testTdf138345_numberingHighlight.
// Their Word fixed PDFs give the directly formatted 12pt paragraph marks a
// 17.28pt line box even though the centered visible Latin text is shorter.
fn mapped_fixture_numbering_shading_direct_paragraph_mark_keeps_word_line_box() {
    let summary = render_summary("tdf135774_numberingShading.docx");
    let line_step = (text_segment_first_origin_y(&summary, 0, "C")
        - text_segment_first_origin_y(&summary, 0, "dd"))
    .abs();
    assert!(
        (line_step - 17.28).abs() <= 0.25,
        "direct paragraph-mark formatting should keep the 17.28pt Word line step, got {line_step}pt"
    );
}

#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport15.cxx:testTdf122648.
// ECMA-376 Part 1 §§17.3.1.13 and 17.18.44 distinguish the physical `left`
// spelling in this package from the logical leading-edge `start` spelling.
fn mapped_fixture_tdf122648_physical_left_keeps_latin_line_box() {
    let summary = render_summary("tdf122648.docx");
    let lines = summary
        .text_segments
        .iter()
        .filter(|segment| segment.page_index == 0)
    ...
    let first_table_row_step = (lines[0].1.bottom - lines[2].1.bottom).abs();
    assert!(
        (first_table_row_step - 20.66).abs() <= 0.25,
        "physical-left table rows should retain Word's Latin line box: {first_table_row_step}pt"
    );

    let second_table_offset = (lines[0].1.bottom - lines[3].1.bottom).abs();
    assert!(
        (second_table_offset - 56.78).abs() <= 0.25,
        "the physical-left separator must not acquire a CJK paragraph-mark line: {second_table_offset}pt"
    );
}

#[test]
// Sources: ECMA-376 Part 1 §17.3.3.2 and §17.18.4 require a column
// break to move the following content to the next available column. LibreOffice
// ooxmlexport9.cxx:testTdf103975_notPageBreakE guards the same fixture against
// losing that break. Office's fixed output also shows that the continuous
// section balances the authored early break at its used height, not at a full
// page-height column.
fn mapped_fixture_tdf103975_early_column_break_uses_compact_balanced_height() {
    let summary = render_summary("tdf103975_notPageBreakE.docx");
    assert_eq!(summary.page_count, 1);

    let first = exact_text_segment_bounds_on_page(&summary, 0, "a");
    let following_section = exact_text_segment_bounds_on_page(&summary, 0, "c");
    let baseline_step = (first.bottom - following_section.bottom).abs();
    assert!(
        (baseline_step - 27.60).abs() <= 0.5,
        "the early column break must not make the balanced section page-height: a={first:?}, c={following_section:?}, step={baseline_step}"
    );
}

#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport24.cxx:
// testTdf121670_columnsInSectionsOnly. ECMA-376 Part 4 §14.8.3.27 says a
// continuous section break normally balances the preceding multi-column
// section. The following one-column paragraph starts at that balanced section
// frame edge; the preceding paragraph's full lower spacing must not escape the
// frame and move it down.
fn mapped_fixture_tdf121670_balanced_columns_close_before_continuous_section() {
    let summary = render_summary("tdf121670_columnsInSectionsOnly.docx");
    assert_eq!(summary.page_count, 2);
    assert_text_top_from_page_top_close(&summary, 1, "columns?", 134.93, 0.75);
    // The focused debug renderer's host-font top differs from the calibrated
    // release golden lane by about 1.1pt; the former bug moved this line by
    // 6.64pt, so the bounded 1.25pt window remains a strict handoff guard.
    assert_text_top_from_page_top_close(&summary, 1, "If it can handle", 153.67, 1.25);
}

#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport12.cxx:testTdf143510
// Source: ../core/sw/qa/extras/uiwriter/uiwriter10.cxx:testRedlineDOCXTableMoveToFrame
fn mapped_fixture_tc_table_dnd_move_keeps_hidden_source_rows_and_compact_target_rows() {
    let summary = render_summary("sw/qa/extras/ooxmlexport/data/TC-table-DnD-move.docx");

    // Word's final-view golden hides the moveFrom text but retains the source
    // table's document-default line and lower spacing before the body text.
    // The focused renderer's host-font glyph box can differ from the corpus
    // runner by about 1.2pt; the old collapsed source rows missed both origins
    // by more than 17pt, so a 2pt window remains a strict regression guard.
    assert_text_top_from_page_top_close(&summary, 0, "Text", 120.62, 2.0);
    assert_text_top_from_page_top_close(&summary, 0, "A1", 147.14, 2.0);

    // The visible moveTo table remains under Table Grid's compact 240/240,
    // after=0 paragraph metrics; do not apply the source-row exception to it.
    let first = exact_text_segment_bounds_on_page(&summary, 0, "A1");
    let second = exact_text_segment_bounds_on_page(&summary, 0, "A2");
    assert!(
        ((first.bottom - second.bottom).abs() - 13.92).abs() <= 0.25,
        "move-to rows should retain their compact baseline step: A1={first:?}, A2={second:?}"
    );
}
...
fn mapped_fixture_customshape_position_keeps_vertical_offset() {
    ...
    // Office keeps the following heading in the usable right-hand segment of
    // the tight wrap. Expanding the first-line collision into its upper
    // margin must not turn this side wrap into a full-width dummy line.
    let media_box = parse_pdf_rect(&summary.media_boxes[0]).unwrap();
    let overview = exact_text_segment_bounds_on_page(&summary, 0, "Overview");
    assert!(
        (overview.left - 137.3).abs() <= 3.0,
        "Overview should remain beside the tight-wrapped picture; bounds={overview:?}"
    );
    let top_from_page_top = media_box.top - overview.top;
    assert!(
        (top_from_page_top - 126.53).abs() <= 3.0,
        "Overview should not move below the picture; top={top_from_page_top}pt, bounds={overview:?}"
    );
}
...
fn mapped_fixture_tdf156902_glow_group_is_composited_as_one_visible_image() {
    let summary = render_summary("tdf156902_GlowOnGroup.docx");
    // A group glow is applied after its child paths are composited. Requiring
    // the original child paths in the PDF would reject the bounded full-color
    // raster that matches the Microsoft Office golden.
    assert_page_image_count_at_least(&summary, 0, 1);
}
...
fn mapped_fixture_tdf117923_keeps_numbering_portion_text_and_font_height() {
    let summary = render_summary("sw/qa/extras/layout/data/tdf117923.docx");
    assert_page_contains(&summary, 0, "GHI GHI GHI GHI");
    assert_page_contains(&summary, 0, "2.");
...
fn mapped_fixture_floattable_split_keeps_follow_floating_table_on_second_page() {
    let summary = render_summary("sw/qa/extras/uiwriter/data/floattable-split.docx");
...
fn mapped_fixture_tdf124600_keeps_second_body_paragraph_on_one_line() {
    let summary = render_summary("sw/qa/extras/layout/data/tdf124600.docx");
...
fn mapped_fixture_watermark_keeps_picture_watermark_washout_visible() {
    let summary = render_summary("oox/qa/unit/data/watermark.docx");
    assert_page_image_count(&summary, 0, 1);
    assert_image_centers_are_grayscale("oox/qa/unit/data/watermark.docx", &summary, 0, 1);
}
...
#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport5.cxx:testFDO78284
// Package evidence: word/diagrams/drawing1.xml dsp:style/a:fontRef uses a:schemeClr val="lt1".
fn mapped_fixture_fdo78284_uses_persisted_smartart_font_reference_color() {
    let summary = render_summary("fdo78284.docx");
    assert_text_object_fill_color(&summary, "1", "#ffffff@ff");
}

#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport10.cxx:testSmartartStrict
// Counterpart: all three persisted text shapes carry a:fontRef/a:schemeClr val="lt1".
fn mapped_fixture_strict_smartart_uses_persisted_font_reference_color() {
    let summary = render_summary("strict-smartart.docx");
    for text in ["a", "b", "c"] {
        assert_text_object_fill_color(&summary, text, "#ffffff@ff");
    }
}

#[test]
// Source: ../core/sw/qa/extras/ooxmlexport/ooxmlexport9.cxx:testTdf79272_strictDxa
// Package evidence: the nested table's vertically merged master cell stores
// strict w:textDirection w:val="lr" and the visible text "USA".
fn mapped_fixture_tdf79272_keeps_nested_vertical_merge_text() {
    let summary = render_summary("tdf79272_strictDxa.docx");
    assert_page_contains(&summary, 0, "USA");
}

#[test]
// Source: ../core/sw/qa/filter/ascii/ascii.cxx:testTdf162180
// Counterexample pair: the same three-row vertical merge must preserve all
// surrounding horizontal labels as well as its rotated master-cell text.
fn mapped_fixture_tdf162180_keeps_nested_vertical_merge_text() {
    let summary = render_summary("sw/qa/filter/ascii/data/tdf162180.docx");
    assert_page_contains(&summary, 0, "USA");
    for text in ["West", "Approved", "Central", "Pending", "East"] {
