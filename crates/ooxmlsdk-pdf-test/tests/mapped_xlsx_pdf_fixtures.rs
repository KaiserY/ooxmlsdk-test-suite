use ooxmlsdk_pdf_test::{
    LinkTargetKind, PdfSummary, libreoffice_fixture, parse_pdf_rect, pdf_summary_for_fixture,
};
...
fn assert_media_box(
    ...
    assert!(
        (bounds.right - expected_width).abs() <= 0.05
            && (bounds.top - expected_height).abs() <= 0.05,
...
fn mapped_xlsx_radio_buttons_keep_grouped_button_labels_visible() {
    ...
    // The grouped controls are on the second horizontal print page in the
    // Microsoft Office fixed-output reference.
    assert_page_contains(&summary, 1, "Group Box 7");
    assert_page_contains(&summary, 1, "Form groupbox1");
...
fn mapped_xlsx_pivot_format_data_field_in_row_keeps_values_visible() {
    ...
    assert_page_contains(&summary, 0, "Values 1 2");
