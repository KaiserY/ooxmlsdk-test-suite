...
use ooxmlsdk_pdf_test::{PdfSummary, libreoffice_fixture, pdf_summary_for_fixture};
...
fn assert_has_path_stroke_color(summary: &PdfSummary, expected: &str) {
    assert!(
        summary.paths.iter().any(
            |path| path.stroke_color.as_deref() == Some(expected) && path.stroked == Some(true)
        ),
...
fn core_docx_pdf_fixture_wordart2_preserves_fill_and_stroke_colors() {
    ...
    // Word fixed output converts warped WordArt glyphs to paths.
    assert_has_path_fill_rgb(&summary, "#d7e4bd");
...
fn core_docx_pdf_fixture_wordart_non_accent_colors_preserve_all_expected_fills() {
    ...
    for expected in ["#ffcc99@ff", "#ff0000@ff", "#ebddc3@ff", "#775f55@ff"] {
        assert_has_path_fill_rgb(&summary, color_rgb(expected));
    }
}
