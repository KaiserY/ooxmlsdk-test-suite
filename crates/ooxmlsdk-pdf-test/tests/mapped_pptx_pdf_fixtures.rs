...
fn assert_has_stroked_path_color(summary: &PdfSummary, expected: &str) {
    assert!(
        summary.paths.iter().any(
            |path| path.stroked == Some(true) && path.stroke_color.as_deref() == Some(expected)
        ),
...
fn assert_has_path_fill_color(summary: &PdfSummary, expected_color: &str) {
    assert!(
        summary
            .paths
            .iter()
            .any(|path| path.fill_mode.is_some()
                && path.fill_color.as_deref() == Some(expected_color)),
...
fn assert_page_link_count_at_least(summary: &PdfSummary, page_index: usize, expected: usize) {
    let count = summary
        ...
        + summary
            ...
            .iter()
            .filter(|annotation| {
                annotation.page_index == page_index && annotation.action_uri.is_some()
            })
            .count();
    assert!(
        count >= expected,
    ...
    assert!(
        summary.annotations.iter().any(|annotation| {
            annotation.page_index == page_index
                && annotation.action_uri.as_deref() == Some(expected_uri)
...
fn mapped_pptx_tdf89927_preserves_white_text_color() {
    ...
    let rendered = rendered_page_image_for_fixture(&fixture("pptx/tdf89927.pptx"), 0, 1200)
        .expect("render tdf89927");
    let visible_white_pixels = rendered
        .rgba
        .chunks_exact(4)
        .enumerate()
        .filter(|(_, pixel)| {
        ...
        .map(|(index, _)| {
            (
                index % rendered.width_px as usize,
                index / rendered.width_px as usize,
            )
        })
        .collect::<Vec<_>>();
    assert!(
        visible_white_pixels.len() >= 100,
        "TEST must be visibly painted on the black slide, found only {} white pixels",
        visible_white_pixels.len()
    );
    let min_x = visible_white_pixels.iter().map(|(x, _)| *x).min().unwrap();
    let max_x = visible_white_pixels.iter().map(|(x, _)| *x).max().unwrap();
    let min_y = visible_white_pixels.iter().map(|(_, y)| *y).min().unwrap();
    let max_y = visible_white_pixels.iter().map(|(_, y)| *y).max().unwrap();
    assert!(
        min_x >= 800 && max_x <= 1100 && min_y >= 350 && max_y <= 550,
        "visible TEST pixels must stay in the lower-right title frame, got x={min_x}..{max_x}, y={min_y}..{max_y}"
    );
}
...
fn mapped_pptx_smartart_pyramid_one_child_preserves_child_text() {
    ...
    assert!(
        summary.paths.iter().any(|path| {
            path.page_index == 0
                && path.segments == 5
                && path.fill_color.as_deref() == Some("#4472c4@ff")
                && path
                    .segment_details
                    .get(1)
                    .zip(path.segment_details.get(2))
                    .is_some_and(|(first, second)| first.x == second.x && first.y == second.y)
                && path
                    .bounds
                    .as_deref()
                    .and_then(|bounds| parse_pdf_rect(bounds).ok())
                    .is_some_and(|bounds| bounds.width() >= 600.0 && bounds.height() >= 400.0)
        }),
        "missing full-size triangular SmartArt level; paths={:?}",
        summary.paths
    );
}
...
#[test]
// Source: ../core/sd/qa/unit/export-tests-ooxml4.cxx:testTdf125551
fn mapped_pptx_tdf125551_rotates_smartart_fallback_rectangles() {
    let summary = render_summary("pptx/tdf125551.pptx");
    let horizontal_blocks = summary
        .paths
        .iter()
        .filter(|path| path.page_index == 0)
        .filter_map(|path| path.bounds.as_deref())
        .filter_map(|bounds| parse_pdf_rect(bounds).ok())
        .filter(|bounds| bounds.width() >= 230.0 && bounds.height() <= 100.0)
        .count();
    assert!(
        horizontal_blocks >= 3,
        "expected three horizontal SmartArt blocks, got {horizontal_blocks}; paths={:?}",
        summary.paths
    );
}
...
#[test]
// Source: ../core/sd/qa/unit/export-tests-ooxml2.cxx:testTdf114845_rotateShape
fn mapped_pptx_tdf114845_preserves_flipped_and_zero_width_connectors() {
    let summary = render_summary("pptx/tdf114845_rotateShape.pptx");
    assert_page_text_occurs_at_least(&summary, 0, "Test", 3);
    assert_page_stroked_path_count_at_least(&summary, 0, 3);
}
...
fn mapped_pptx_theme_preserves_initial_theme_colored_text_output() {
    let summary = render_summary("sd/qa/unit/data/theme.pptx");
    assert_full_render_reference_smoke(&summary);
    assert_has_text_fill_color(&summary, "#4472c4@ff");
}
...
#[test]
// Source: ../core/sd/qa/unit/activex-controls-tests.cxx:testPictureProperties
// Format: [MS-OI29500] Part 1 §19.3.2.1 defines p:control imgW/imgH as
// PowerPoint's thumbnail dimensions.
fn mapped_pptx_activex_picture_keeps_native_wmf_button_text_searchable() {
    let summary = render_summary("pptx/activex_picture.pptx");
    assert_eq!(
        normalize_space(&page_text(&summary, 0)),
        "CommandButton1 CommandButton CommandButton CommandButton CommandButton CommandButton CommandButton CommandButton1 CommandButton1 CommandButton1 CommandButton1 CommandButton1 CommandButton1"
    );
}
