...
pub fn render_fixture_pdf_with_options(fixture: &Path, options: PdfOptions) -> Result<Vec<u8>> {
    ...
    match normalized_extension(fixture).as_deref() {
        Some("pptx" | "pptm" | "ppsx" | "ppsm" | "potx" | "potm") => {
            Ok(convert_pptx(file, options)?)
        }
        Some("xlsx" | "xlsm" | "xltx" | "xltm") => Ok(convert_xlsx(file, options)?),
...
pub fn render_fixture_pdf_with_diagnostics(
    ...
    match normalized_extension(fixture).as_deref() {
        ...
        Some("xlsx" | "xlsm" | "xltx" | "xltm") => {
            Ok(convert_xlsx_with_diagnostics(file, options)?)
        }
...
pub fn render_fixture_pdf_with_font_audit(
    ...
    match normalized_extension(fixture).as_deref() {
        Some("pptx" | "pptm" | "ppsx" | "ppsm" | "potx" | "potm") => {
            Ok(convert_pptx_with_font_audit(file, options)?)
        }
        Some("xlsx" | "xlsm" | "xltx" | "xltm") => Ok(convert_xlsx_with_font_audit(file, options)?),
