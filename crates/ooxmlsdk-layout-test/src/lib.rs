...
pub fn docx_layout(path: &str) -> Result<LayoutDocument<'static>> {
    docx_layout_with_options(path, &LayoutOptions::default())
}

pub fn docx_layout_with_options(
    path: &str,
    options: &LayoutOptions,
) -> Result<LayoutDocument<'static>> {
    let file = std::fs::File::open(corpus_file(path)).map_err(ooxmlsdk::common::SdkError::from)?;
    let package = WordprocessingDocument::new_with_settings(file, office_open_settings())?;
    ooxmlsdk_layout::docx::layout_document(&package, options)
}
...
pub fn docx_layout_named(name: &str) -> Result<LayoutDocument<'static>> {
    ...
    let package = WordprocessingDocument::new_with_settings(file, office_open_settings())?;
    ooxmlsdk_layout::docx::layout_document(&package, &LayoutOptions::default())
}

pub fn pptx_layout(path: &str) -> Result<LayoutDocument<'static>> {
    let file = std::fs::File::open(corpus_file(path)).map_err(ooxmlsdk::common::SdkError::from)?;
    let package = PresentationDocument::new_with_settings(file, office_open_settings())?;
...
pub fn xlsx_layout(path: &str) -> Result<LayoutDocument<'static>> {
    ...
    let package = SpreadsheetDocument::new_with_settings(file, office_open_settings())?;
...
fn collect_display_items<'a, 'doc>(
    items: &'a [DisplayItem<'doc>],
    flattened: &mut Vec<&'a DisplayItem<'doc>>,
) {
    for item in items {
        flattened.push(item);
        if let DisplayItem::Group(group) = item {
            collect_display_items(&group.items, flattened);
        }
    }
}

pub fn page_display_items<'a, 'doc>(
    document: &'a LayoutDocument<'doc>,
    page_index: usize,
) -> Vec<&'a DisplayItem<'doc>> {
    let page = document
        .pages
        .get(page_index)
        .unwrap_or_else(|| panic!("missing page {page_index}; pages={}", document.pages.len()));
    let mut items = Vec::new();
    collect_display_items(&page.items, &mut items);
    items
}

fn document_display_items<'a, 'doc>(
    document: &'a LayoutDocument<'doc>,
) -> Vec<&'a DisplayItem<'doc>> {
    let mut items = Vec::new();
...
pub fn all_page_text(document: &LayoutDocument<'_>, page_index: usize) -> String {
    let items = page_display_items(document, page_index);
    let text_capacity = items
        .iter()
    ...
    Describe: Fn(&T, String) -> String + Send + Sync + Copy,
{
    ...
    std::thread::scope(|scope| {
        for _ in 0..worker_count {
            ...
            scope.spawn(move || {
                loop {
                    let selected_index = next_case.fetch_add(1, Ordering::Relaxed);
                    let Some(&(case_index, case)) = cases.get(selected_index) else {
                        break;
                    };
...
pub fn text_origins_for<'a>(
    ...
    page_display_items(document, page_index)
        .into_iter()
        .filter_map(|item| match item {
...
pub fn assert_page_stroked_path_count_at_least(
    ...
    let page = document
        ...
        .get(page_index)
        .unwrap_or_else(|| panic!("missing page {page_index}; pages={}", document.pages.len()));
    let count = page_display_items(document, page_index)
        .into_iter()
...
pub fn assert_text_font_size(
    ...
    let text_runs = document_display_items(document)
        .into_iter()
        .filter_map(|item| match item {
...
fn link_targets(document: &LayoutDocument<'_>) -> Vec<String> {
    document_display_items(document)
        .into_iter()
        .filter_map(|item| match item {
...
fn unique_corpus_file_named(name: &str) -> PathBuf {
    ...
    let matches = libreoffice_corpus_files_by_name()
        .get(name)
        .map(Vec::as_slice)
        .unwrap_or_default();
    match matches {
        [path] => path.clone(),
        [] => panic!("missing LibreOffice corpus fixture named {name:?} under {root:?}"),
        _ => panic!("ambiguous LibreOffice corpus fixture named {name:?}: {matches:?}"),
    }
}

fn libreoffice_corpus_files_by_name() -> &'static HashMap<String, Vec<PathBuf>> {
    static FILES_BY_NAME: OnceLock<HashMap<String, Vec<PathBuf>>> = OnceLock::new();
    FILES_BY_NAME.get_or_init(|| {
        let mut files = HashMap::new();
        index_corpus_files_by_name(&corpus_file(""), &mut files);
        files
    })
}

fn index_corpus_files_by_name(dir: &Path, files: &mut HashMap<String, Vec<PathBuf>>) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            index_corpus_files_by_name(&path, files);
        } else if let Some(name) = path.file_name().and_then(|file_name| file_name.to_str()) {
            files.entry(name.to_owned()).or_default().push(path);
        }
    }
}
