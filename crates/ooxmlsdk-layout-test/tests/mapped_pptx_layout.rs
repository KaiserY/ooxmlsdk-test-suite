...
fn pptx_embedded_font_typeface_is_imported_into_layout_summary() {
    ...
    let package = PresentationDocument::new_from_file(corpus_file(
        "sd/qa/unit/data/BoldonseFontEmbedded.pptx",
    ))
    .expect("Boldonse embedded-font fixture should open");
    let summary =
        ooxmlsdk_layout::pptx::inspect_layout(&package).expect("PPTX layout should inspect");
...
#[test]
fn pptx_activex_live_back_color_reaches_the_metafile_preview() {
    // Sources:
    // - sd/qa/unit/import-tests2.cxx::testActiveXToggleButton
    // - [MS-OFORMS] MorphDataControl BackColor
    let document = pptx_layout("sd/qa/unit/data/pptx/activex_togglebutton.pptx")
        .expect("ActiveX toggle-button fixture should lay out");
    let palettes = document.pages[0]
        .items
        .iter()
        .filter_map(|item| match item {
            DisplayItem::Image(image) => image.metafile_monochrome_dib_palette_override,
            _ => None,
        })
        .collect::<Vec<_>>();
...
#[test]
fn pptx_activex_scroll_bar_colors_reach_the_metafile_preview() {
    // Sources:
    // - sd/qa/unit/activex-controls-tests.cxx::testScrollBarProperties
    // - [MS-OFORMS] ScrollBarControl BackColor and VariousPropertyBits
    let document = pptx_layout("sd/qa/unit/data/pptx/activex_scrollbar.pptx")
        .expect("ActiveX scroll-bar fixture should lay out");
    let palettes = document.pages[0]
        .items
        .iter()
        .filter_map(|item| match item {
