use ooxmlsdk_layout_test::{
    ...
    assert_path_width_count, docx_layout, docx_layout_named, page_display_items,
    run_named_cases_parallel,
};
...
const CASES: &[DocxCase] = &[
    ...
    case!(
        tdf160518_moves_page_bottom_hyphenated_line,
        source: "../core/sw/qa/extras/ooxmlexport/ooxmlexport23.cxx:testTdf160518",
        file: "tdf160518_useWord2013TrackBottomHyphenation.docx",
        pages: 3,
    ),
    case!(
        tdf160518_allows_page_bottom_hyphenated_line,
        source: "../core/sw/qa/extras/ooxmlexport/ooxmlexport23.cxx:testTdf160518_compatible",
        file: "tdf160518_allowHyphenationAtTrackBottom.docx",
        pages: 2,
    ),
    case!(
        ...
        file: "tdf165354.docx",
        contains: [pt!(0, "except that it has an at-"), pt!(1, "mosphere. The Earth")],
    ),
    case!(
    ...
    case!(
        ...
        contains: [pt!(0, "Interní / Internal")],
        path_minimums: [page_count!(0, 1)],
    ),
    case!(
        tdf153042_no_tab,
...
#[test]
fn tdf125885_wordart3_retains_real_text_warp_instead_of_fake_shape_path() {
    let document = docx_layout_named("tdf125885_WordArt3.docx").unwrap();
    let warped_happy = page_display_items(&document, 0).into_iter().filter(|item| {
        let ooxmlsdk_layout::common::DisplayItem::Text(text) = item else {
            return false;
        };
        text.text.contains("Happy")
            && text
                .style
    ...
    assert_eq!(
        warped_happy.count(),
        3,
        "all three DrawingML WordArt text bodies must retain their textCurveDown warp without counting the compatibility copies"
    );
}
