...
fn normalized_family_aliases_resolve_before_matching() {
    ...
    assert_eq!(
        ...
        Some(FontSubstitutionReason::Alias)
    );
}
...
#[test]
fn missing_family_and_missing_glyph_use_distinct_policy_chains() {
    // Sources:
    // - LibreOffice vcl/source/font/PhysicalFontCollection.cxx
    //   FindFontFamily() and GetGlyphFallbackFont().
    // - Skia src/ports/SkFontMgr_win_dw.cpp onMatchFamilyStyle() and onFallback().
    // - ReactOS win32ss/gdi/ntgdi/freetype.c TextIntRealizeFont() and
    //   FontLink_Chain_FindGlyph().
    let mut registry = FontRegistry::new();
    let mut primary = FontFaceInfo::synthetic("primary", "Primary Substitute");
    primary.coverage = coverage_for_chars(['A']);
    registry.register_face(FontSource::System, primary);
    let mut glyph_fallback = FontFaceInfo::synthetic("symbol", "Glyph Fallback");
    glyph_fallback.coverage = coverage_for_chars(['\u{2610}']);
    registry.register_face(FontSource::System, glyph_fallback);
    registry
        .book
        .family_substitution_chains
        .push(FontFallbackChain {
            requested_family: Some(Cow::Borrowed("Missing Family")),
            script: None,
            language: None,
            families: vec![Cow::Borrowed("Primary Substitute")],
        });
    registry.book.fallback_chains.push(FontFallbackChain {
        requested_family: None,
        script: None,
        language: None,
        families: vec![Cow::Borrowed("Glyph Fallback")],
    });
    ...
    let resolved = registry.resolve(&request).expect("family substitution");
    assert_eq!(resolved.font_id, FontId(Arc::from("primary")));
...
#[test]
fn word_small_caps_reduce_lowercase_graphemes_but_not_digits_or_punctuation() {
    // Sources:
    // - ISO/IEC 29500-1 §17.3.2.33 says w:smallCaps does not affect
    //   non-alphabetic characters.
    // - LibreOffice sw/source/core/txtnode/fntcap.cxx selects only
    //   CharType::LOWERCASE_LETTER blocks for its reduced font.
    // - The Office fixed-format stress015.docx reference keeps "03.09" at
    //   the authored 16pt while reducing lowercase letters to 80%.
    let text = "a,1 b\u{301}中";
    let runs = script_direction_runs(text, FontSize(10.0), true);
    let segments = runs
        .iter()
        .map(|run| (&text[run.text_range.clone()], run.small_caps))
        .collect::<Vec<_>>();
